...
import { invalidateUsageCache as invalidateUsageCacheDefault, readClaudeCliUsageSnapshot as readClaudeCliUsageSnapshotDefault, readClaudeOauthUsageSnapshot as readClaudeOauthUsageSnapshotDefault, readClaudeUsageSnapshot as readClaudeUsageSnapshotDefault, } from "./claudeUsage";
...
export const createApiServer = ({ workspaceCwd, projectStateDir, promptsDir, webDistDir, apiBaseUrl, gitClient, readClaudeUsageSnapshot, readClaudeOauthUsageSnapshot, readClaudeCliUsageSnapshot, readCodexUsageSnapshot = readCodexUsageSnapshotDefault, readGithubRepoSummary, scanUsageHeatmap, monitorService, invalidateClaudeUsageCache = invalidateUsageCacheDefault, allowRemoteAccess = false, } = {}) => {
    ...
    const readClaudeUsageSnapshotWithDefault = readClaudeUsageSnapshot ??
        (() => readClaudeUsageSnapshotDefault({
            projectStateDir: resolvedStateDir,
            backgroundRefreshOnly: true,
        }));
    const readClaudeOauthUsageSnapshotWithDefault = readClaudeOauthUsageSnapshot ??
        (() => readClaudeOauthUsageSnapshotDefault({
            projectStateDir: resolvedStateDir,
        }));
    const readClaudeCliUsageSnapshotWithDefault = readClaudeCliUsageSnapshot ??
        (() => readClaudeCliUsageSnapshotDefault({
            projectStateDir: resolvedStateDir,
        }));
    const readGithubRepoSummaryWithDefault = readGithubRepoSummary ??
    ...
    const monitorServiceWithDefault = monitorService ??
        createMonitorService({
            projectStateDir: resolvedStateDir,
        });
    const scanUsageHeatmapWithDefault = scanUsageHeatmap ??
        ((scope) => scanClaudeUsageChart(scope, resolvedWorkspaceCwd));
