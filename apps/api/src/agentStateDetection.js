...
const OSC_MARKER = 93;
const ST_MARKER = 92;
const stripAnsiAndControlSequences = (value) => {
    let cleaned = "";
...
const findLastRelevantMatchIndex = (text, pattern, chunkStartIndex) => {
...
const hasProcessingSignal = (text, chunkStartIndex) => findLastRelevantMatchIndex(text, PROCESSING_PATTERN, chunkStartIndex) !== -1;
export class AgentStateTracker {
    ...
    state;
    idleDeadlineAt = null;
    constructor({ initialState = "idle", maxBufferLength = DEFAULT_MAX_BUFFER_LENGTH, idleAfterMs = DEFAULT_IDLE_AFTER_MS, } = {}) {
    ...
    forceState(nextState, now = Date.now()) {
    ...
    observeSubmit(now = Date.now()) {
        return this.enterProcessing(now);
    }
    observeChunk(chunk, now = Date.now()) {
        if (!chunk) {
            return null;
        }
...
//# sourceMappingURL=agentStateDetection.js.map