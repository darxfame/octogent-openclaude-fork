...
import { AgentStateTracker } from "../agentStateDetection";
import { DEFAULT_AGENT_PROVIDER, TERMINAL_BOOTSTRAP_COMMANDS, TERMINAL_MAX_CONCURRENT_SESSIONS, TERMINAL_SCROLLBACK_MAX_BYTES, TERMINAL_SESSION_IDLE_GRACE_MS, } from "./constants";
import { ensureTranscriptDirectory, transcriptFilenameForSession, } from "./conversations";
...
const BROKEN_OSC_TAIL_RE = new RegExp(`^\\][^${ANSI_BEL}${ANSI_ESCAPE}]*(?:${ANSI_BEL}|${ANSI_ESCAPE}\\\\)`);
export const createSessionRuntime = ({ websocketServer, terminals, sessions, resolveTerminalSession, getTentacleWorkspaceCwd, isDebugPtyLogsEnabled, ptyLogDir, transcriptDirectoryPath, sessionIdleGraceMs = TERMINAL_SESSION_IDLE_GRACE_MS, scrollbackMaxBytes = TERMINAL_SCROLLBACK_MAX_BYTES, maxConcurrentSessions = TERMINAL_MAX_CONCURRENT_SESSIONS, onStateChange, onSessionStart, onSessionEnd, }) => {
    ...
    const createDebugLog = (sessionId) => {
    ...
    const appendDebugLog = (session, line) => {
        session.debugLog?.write(`${new Date().toISOString()} ${line}\n`);
    };
    const createTranscriptLog = (sessionId) => {
    ...
    const appendTranscriptEvent = (session, sessionId, event) => {
    ...
    const closeTranscript = (session, sessionId, event) => {
    ...
    const emitStateIfChanged = (session, sessionId, nextState) => {
    ...
    const resolveSession = resolveTerminalSession ??
        ((terminalId) => {
    ...
    const clearIdleCloseTimer = (session) => {
    ...
    const clearPromptTimers = (session) => {
    ...
    const schedulePromptTimer = (session, sessionId, callback, delayMs) => {
    ...
    const appendScrollback = (session, chunk) => {
        let nextChunk = chunk;
    ...
    const stripBrokenLeadingAnsi = (text) => {
        let nextText = text;
    ...
    const sendHistory = (websocket, session) => {
    ...
    const teardownSession = (sessionId, session, event, options) => {
        if (session.isClosed) {
            return;
        }
    ...
    const closeSession = (sessionId) => {
        ...
        teardownSession(sessionId, session, {
            type: "session_end",
    ...
    const killSession = (sessionId, signal = "SIGKILL") => {
        const session = sessions.get(sessionId);
        if (!session) {
            return false;
        }
    ...
    const scheduleIdleCloseIfNeeded = (session, sessionId) => {
        if (session.isClosed || sessions.get(sessionId) !== session) {
            return;
        }
    ...
    const ensureAgentBootstrapped = (sessionId, session) => {
        ...
        const bootstrapCommand = TERMINAL_BOOTSTRAP_COMMANDS[provider] ?? TERMINAL_BOOTSTRAP_COMMANDS[DEFAULT_AGENT_PROVIDER];
    ...
    const ensureSession = (sessionId, tentacleId) => {
        const existingSession = sessions.get(sessionId);
        if (existingSession) {
            return existingSession;
        }
        if (sessions.size >= sessionLimit) {
            throw new Error(`Terminal session limit reached (${sessionLimit}). Close an existing terminal session or increase OCTOGENT_MAX_TERMINAL_SESSIONS.`);
        }
        ...
        try {
            ...
            throw new Error(`Unable to start terminal shell (${shellLaunch.command}): ${toErrorMessage(error)}`);
        }
        ...
        const processId = typeof pty.pid === "number" && Number.isInteger(pty.pid) && pty.pid > 0 ? pty.pid : undefined;
            ...
            websocket.on("message", (raw) => {
                ...
                try {
                    ...
                    if (payload.type === "input" && typeof payload.data === "string") {
                        appendDebugLog(session, `ws-input session=${sessionId} data=${JSON.stringify(payload.data)}`);
                        session.pty.write(payload.data);
                        if (/[\r\n]/.test(payload.data)) {
                            emitStateIfChanged(session, sessionId, session.stateTracker.observeSubmit(Date.now()));
                        }
                        return;
                    }
    ...
    const connectDirect = (terminalId, listener) => {
        const resolvedSession = resolveSession(terminalId);
        if (!resolvedSession) {
            return null;
        }
    ...
    const writeInput = (terminalId, data) => {
    ...
    const resizeSession = (terminalId, cols, rows) => {
        const session = sessions.get(terminalId);
        if (!session || session.isClosed) {
            return false;
        }
    ...
    const releaseSessionKeepAlive = (terminalId) => {
        const session = sessions.get(terminalId);
        if (!session || session.isClosed) {
            return false;
        }
