...
const parseRuntimeState = (value) => value === "idle" || value === "processing" ? value : null;
export const transcriptFilenameForSession = (sessionId) => `${encodeURIComponent(sessionId)}.jsonl`;
const parseTranscriptEvent = (value) => {
    if (!isRecord(value)) {
        return null;
    }
    ...
    if (eventType === "session_end") {
        ...
        const signal = typeof value.signal === "number" || typeof value.signal === "string"
            ? value.signal
            : undefined;
        return {
            type: "session_end",
...
const buildConversationSummary = (sessionId, events, turns) => {
    const userTurns = turns.filter((turn) => turn.role === "user");
    const assistantTurns = turns.filter((turn) => turn.role === "assistant");
    const firstEvent = events[0] ?? null;
    const lastEvent = events.length > 0 ? events[events.length - 1] : null;
    const lastSessionEnd = [...events].reverse().find((event) => event.type === "session_end") ?? null;
    return {
        sessionId,
        ...
        lastUserTurnPreview: userTurns.length > 0 ? (userTurns[userTurns.length - 1]?.content ?? null) : null,
        lastAssistantTurnPreview: assistantTurns.length > 0
            ? (assistantTurns[assistantTurns.length - 1]?.content ?? null)
            : null,
    };
};
const parseTranscriptLines = (rawJsonl) => {
    const events = [];
...
const readSessionEventsFromFile = (transcriptDirectoryPath, sessionId) => {
...
const parseSessionIdFromFilename = (filename) => {
...
export const readConversationSession = (transcriptDirectoryPath, sessionId) => {
...
export const listConversationSessions = (transcriptDirectoryPath) => {
...
export const ensureTranscriptDirectory = (transcriptDirectoryPath) => {
    mkdirSync(transcriptDirectoryPath, { recursive: true });
};
export const deleteConversation = (transcriptDirectoryPath, sessionId) => {
...
export const deleteAllConversations = (transcriptDirectoryPath) => {
    if (!existsSync(transcriptDirectoryPath)) {
        return;
    }
...
const claudeTurnsFilename = (sessionId) => `${encodeURIComponent(sessionId)}.claude-turns.json`;
export const storeClaudeTranscriptTurns = (transcriptDirectoryPath, sessionId, turns) => {
...
const readClaudeTranscriptTurns = (transcriptDirectoryPath, sessionId) => {
    const filePath = join(transcriptDirectoryPath, claudeTurnsFilename(sessionId));
...
const buildSearchSnippet = (content, query, contextChars = 80) => {
...
export const searchConversations = (transcriptDirectoryPath, query) => {
...
export const conversationExportMarkdown = (conversation) => {
    const lines = [];
