...
const runGitCommand = (cwd, args) => execFileSync("git", args, {
    cwd,
    encoding: "utf8",
    stdio: "pipe",
}).trim();
const readOptionalGitCommand = (cwd, args) => {
    try {
...
const isExitCode = (error, exitCode) => {
...
const parseDiffNumstatLineCounts = (numstatOutput) => numstatOutput
    .split("\n")
    .map((line) => line.trim())
...
const runGhCommand = (cwd, args) => execFileSync("gh", args, {
    cwd,
    encoding: "utf8",
    stdio: "pipe",
}).trim();
const readErrorDetails = (error) => {
    if (typeof error !== "object" || error === null) {
        return "";
    }
...
const readGhFailureMessage = (error) => {
    if (typeof error === "object" && error !== null) {
...
const isNoPullRequestError = (error) => {
    const details = readErrorDetails(error).toLowerCase();
    return (details.includes("no pull requests found") ||
...
const parsePullRequestPayload = (payload) => {
    ...
    const state = stateRaw === "OPEN" || stateRaw === "MERGED" || stateRaw === "CLOSED" ? stateRaw : "OPEN";
...
const readPullRequestWithGh = (cwd, args) => {
...
export const createDefaultGitClient = () => ({
    assertAvailable() {
        try {
    ...
    removeBranch({ cwd, branchName }) {
        const output = execFileSync("git", ["branch", "--list", "--format=%(refname:short)", branchName], {
            cwd,
            encoding: "utf8",
            stdio: "pipe",
        });
...
//# sourceMappingURL=systemClients.js.map