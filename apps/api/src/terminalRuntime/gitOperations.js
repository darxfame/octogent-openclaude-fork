...
export const createGitOperations = (deps) => {
    ...
    const resolveWorktreeTentacleContext = (tentacleId) => {
        ...
        if (terminal.workspaceMode !== "worktree") {
            throw new RuntimeInputError("Git lifecycle actions are only available for worktree terminals.");
        }
        return {
            terminal,
            workspaceCwd: worktreeManager.getTentacleWorkspaceCwd(terminal.worktreeId ?? terminal.tentacleId),
        };
    };
    const readWorktreeGitStatus = (tentacleId, terminal, workspaceCwd) => {
        try {
            ...
            throw new RuntimeInputError(`Unable to read git status for ${tentacleId}: ${error instanceof Error ? error.message : String(error)}`);
        }
    };
    const toPullRequestStatus = (state) => state === "OPEN" ? "open" : state === "MERGED" ? "merged" : "closed";
    const emptyPullRequestSnapshot = (tentacleId, terminal) => ({
        tentacleId,
        workspaceMode: terminal.workspaceMode,
        status: "none",
        number: null,
    ...
    const readWorktreePullRequest = (tentacleId, terminal, workspaceCwd) => {
        try {
            ...
            throw new RuntimeInputError(`Unable to read pull request for ${tentacleId}: ${error instanceof Error ? error.message : String(error)}`);
        }
    };
    return {
        readTentacleGitStatus(tentacleId) {
        ...
        commitTentacleWorktree(tentacleId, message) {
            const context = resolveWorktreeTentacleContext(tentacleId);
            if (!context) {
                return null;
            }
            ...
            try {
                ...
                throw new RuntimeInputError(`Unable to commit ${tentacleId}: ${error instanceof Error ? error.message : String(error)}`);
            }
            return readWorktreeGitStatus(tentacleId, context.terminal, context.workspaceCwd);
        },
        pushTentacleWorktree(tentacleId) {
            const context = resolveWorktreeTentacleContext(tentacleId);
            if (!context) {
                return null;
            }
            try {
                ...
                throw new RuntimeInputError(`Unable to push ${tentacleId}: ${error instanceof Error ? error.message : String(error)}`);
            }
            return readWorktreeGitStatus(tentacleId, context.terminal, context.workspaceCwd);
        },
        syncTentacleWorktree(tentacleId, baseRef) {
            const context = resolveWorktreeTentacleContext(tentacleId);
            if (!context) {
                return null;
            }
            const statusBeforeSync = readWorktreeGitStatus(tentacleId, context.terminal, context.workspaceCwd);
            ...
            const effectiveBaseRef = normalizedBaseRef && normalizedBaseRef.length > 0
                ? normalizedBaseRef
            ...
            try {
                ...
                throw new RuntimeInputError(`Unable to sync ${tentacleId}: ${error instanceof Error ? error.message : String(error)}`);
            }
            return readWorktreeGitStatus(tentacleId, context.terminal, context.workspaceCwd);
        },
        readTentaclePullRequest(tentacleId) {
            const context = resolveWorktreeTentacleContext(tentacleId);
            if (!context) {
                return null;
            }
        ...
        createTentaclePullRequest(tentacleId, input) {
            const context = resolveWorktreeTentacleContext(tentacleId);
            if (!context) {
                return null;
            }
            ...
            const existingPullRequest = readWorktreePullRequest(tentacleId, context.terminal, context.workspaceCwd);
            ...
            try {
                ...
                throw new RuntimeInputError(`Unable to create pull request for ${tentacleId}: ${error instanceof Error ? error.message : String(error)}`);
            }
        },
        mergeTentaclePullRequest(tentacleId) {
            const context = resolveWorktreeTentacleContext(tentacleId);
            if (!context) {
                return null;
            }
            const currentPullRequest = readWorktreePullRequest(tentacleId, context.terminal, context.workspaceCwd);
            ...
            try {
                ...
                throw new RuntimeInputError(`Unable to merge pull request for ${tentacleId}: ${error instanceof Error ? error.message : String(error)}`);
            }
            return readWorktreePullRequest(tentacleId, context.terminal, context.workspaceCwd);
        },
    };
