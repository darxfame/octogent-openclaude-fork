...
const deriveTerminalNameFromPrompt = (prompt) => {
...
export const createHookProcessor = (deps) => {
    const { terminals, sessions, transcriptDirectoryPath, getApiBaseUrl, persistRegistry, deliverChannelMessages, releaseSessionKeepAlive, onStateChange, } = deps;
    const parseSettingsObject = (fileContents) => {
        try {
    ...
    const mergeHookEntries = (existingValue, eventName, nextEntries) => {
        const nextHooks = existingValue && typeof existingValue === "object" && !Array.isArray(existingValue)
            ? { ...existingValue }
            : {};
        ...
        for (const nextEntry of nextEntries) {
            ...
            const alreadyPresent = existingEntries.some((existingEntry) => JSON.stringify(existingEntry) === serializedNextEntry);
            if (!alreadyPresent) {
    ...
    const installHooksInDirectory = (targetCwd) => {
        ...
        try {
            ...
            let mergedHooks = mergedSettings.hooks &&
    ...
    const handleHook = (hookName, payload, octogentSessionId) => {
        ...
        if (hookName === "notification") {
            ...
            const notificationType = typeof hookPayloadRecord.notification_type === "string"
        ...
        if (hookName === "pre-tool-use") {
            ...
            const toolName = typeof hookPayloadRecord.tool_name === "string" ? hookPayloadRecord.tool_name : null;
        ...
        if (hookName === "user-prompt-submit") {
            ...
            if (terminal.nameOrigin === "generated") {
                const prompt = typeof hookPayloadRecord.prompt === "string" ? hookPayloadRecord.prompt.trim() : "";
        ...
        const transcriptPath = typeof hookPayload.transcript_path === "string" ? hookPayload.transcript_path : null;
        ...
        if (octogentSessionId && sessions.has(octogentSessionId)) {
            ...
            logVerbose(`[Hook] octogent_session=${octogentSessionId} not found in active sessions, skipping.`);
            return { ok: true };
        }
        else {
        ...
        const lastAssistantMessage = typeof hookPayload.last_assistant_message === "string"
...
//# sourceMappingURL=hookProcessor.js.map