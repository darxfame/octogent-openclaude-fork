...
const GITHUB_REPOSITORY_QUERY = "query($owner:String!,$name:String!){repository(owner:$owner,name:$name){nameWithOwner stargazerCount issues(states:OPEN){totalCount} pullRequests(states:OPEN){totalCount}}}";
const unavailableSnapshot = (now, message) => ({
    status: "unavailable",
    fetchedAt: now.toISOString(),
    source: "none",
    message,
});
const errorSnapshot = (now, message) => ({
    status: "error",
    fetchedAt: now.toISOString(),
    source: "none",
    message,
...
const readGhAuthenticationFailure = (error) => {
    ...
    const stderr = typeof errorWithCode.stderr === "string"
...
const readRepository = async (runCommand, cwd, env) => {
    const { stdout } = await runCommand("gh", ["repo", "view", "--json", "nameWithOwner", "--jq", ".nameWithOwner"], {
        cwd,
        env,
    });
    return normalizeRepository(stdout);
};
const readRepositoryStats = async (runCommand, cwd, env, repository) => {
...
const readCommitSeries = async (runCommand, cwd, env, now) => {
...
const parseRecentCommit = (entry) => {
    const [rawHash, rawShortHash, rawAuthorName, rawAuthorEmail, rawAuthoredAt, rawBody, ...subjectParts] = entry.split("\u001f").map((part) => part.trim());
...
const readCommitDiffStats = async (runCommand, cwd, env) => {
    const { stdout } = await runCommand("git", ["log", `-${RECENT_COMMIT_LIMIT}`, "--pretty=format:%H", "--shortstat"], { cwd, env });
    const map = new Map();
...
const readRecentCommits = async (runCommand, cwd, env) => {
...
export const readGithubRepoSummary = async (dependencies = {}) => {
...
//# sourceMappingURL=githubRepoSummary.js.map