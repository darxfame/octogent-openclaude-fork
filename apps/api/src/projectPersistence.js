...
const toProjectRegistryEntry = (value, workspaceCwd) => {
    if (!isRecord(value)) {
        return null;
    }
    ...
    if (workspaceCwd &&
        ...
        return {
            ...
            name: value.name,
            path: value.path,
            createdAt: typeof value.createdAt === "string" && value.createdAt.trim().length > 0
                ? value.createdAt
...
const toProjectConfigDocument = (value) => {
    if (!isRecord(value) ||
        value.version !== 1 ||
...
export const loadProjectsRegistry = () => {
...
export const saveProjectsRegistry = (registry) => {
    ensureGlobalOctogentDir();
    writeFileSync(PROJECTS_FILE, `${JSON.stringify(registry, null, 2)}\n`, "utf8");
};
export const resolveProjectConfigPath = (workspaceCwd) => join(workspaceCwd, PROJECT_CONFIG_RELATIVE_PATH);
export const deriveProjectIdFromWorkspace = (workspaceCwd) => `workspace-${createHash("sha1").update(workspaceCwd).digest("hex").slice(0, 16)}`;
const inferLegacyProjectName = (workspaceCwd) => {
    const parsed = readJsonFile(PROJECTS_FILE);
    if (!isRecord(parsed) || !Array.isArray(parsed.projects)) {
        return null;
    }
    for (const legacyEntry of parsed.projects) {
...
export const loadProjectConfig = (workspaceCwd) => {
...
export const ensureProjectConfig = (workspaceCwd, preferredName, preferredProjectId) => {
...
export const registerProject = (workspaceCwd, preferredName) => {
    ...
    const filteredProjects = registry.projects.filter((entry) => entry.path !== workspaceCwd && entry.id !== projectConfig.projectId);
    filteredProjects.push(nextEntry);
    saveProjectsRegistry({ projects: filteredProjects });
    return nextEntry;
};
export const resolveGlobalProjectDir = (projectId) => join(GLOBAL_OCTOGENT_DIR, "projects", projectId);
export const resolveEphemeralProjectStateDir = (workspaceCwd) => resolveGlobalProjectDir(deriveProjectIdFromWorkspace(workspaceCwd));
export const resolveProjectStateDir = (workspaceCwd, preferredName) => {
...
export const ensureProjectScaffold = (workspaceCwd, preferredName, preferredProjectId) => {
...
export const hasOctogentGitignoreEntry = (workspaceCwd) => {
...
export const ensureOctogentGitignoreEntry = (workspaceCwd) => {
    const gitignorePath = join(workspaceCwd, ".gitignore");
...
export const migrateStateToGlobal = (workspaceCwd, projectStateDir) => {
    ...
    const legacyGlobalProjectDir = legacyProjectName && currentConfig
...
//# sourceMappingURL=projectPersistence.js.map