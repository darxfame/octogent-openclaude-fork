...
const scanJsonlFile = async (filePath, projectLabel, buckets) => {
    let content;
    try {
    ...
    for (const line of content.split("\n")) {
        ...
        bucket.projectTokens.set(projectLabel, (bucket.projectTokens.get(projectLabel) ?? 0) + totalTokens);
...
const scanProjectDirectory = async (projectDir, projectLabel, buckets) => {
    let entries;
    try {
    ...
    await Promise.all(jsonlFiles.map((file) => scanJsonlFile(join(projectDir, file), projectLabel, buckets)));
};
...
const sortedKeys = (totals) => Array.from(totals.entries())
    .sort((a, b) => b[1] - a[1])
    .map(([name]) => name);
const mapToSlices = (map) => Array.from(map.entries())
    .map(([key, tokens]) => ({ key, tokens }))
    .sort((a, b) => b.tokens - a.tokens);
const projectSlugFromCwd = (cwd) => cwd.replace(/\//g, "-");
let cachedResult = null;
const CACHE_TTL_MS = 120_000;
export const scanClaudeUsageChart = async (scope, workspaceCwd) => {
    ...
    if (scope === "project" && projectSlug) {
        ...
        await Promise.all(projectDirs.map((dir) => scanProjectDirectory(join(CLAUDE_PROJECTS_DIR, dir), slugToLabel(dir), buckets)));
    }
