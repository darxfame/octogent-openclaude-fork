...
const mapUsageResponse = (response, now) => {
...
const loadCredentials = (authJson) => {
...
const shouldRefreshToken = (credentials, now) => {
...
const refreshCredentials = async (authPath, authJson, credentials, now, dependencies) => {
    if (!credentials.refreshToken) {
        return credentials;
    }
    ...
    const refreshedRefreshToken = asString(refreshPayload.refresh_token)?.trim() || credentials.refreshToken;
    const mergedTokens = {
...
export const readCodexUsageSnapshot = async (dependencies = {}) => {
    const env = dependencies.env ?? process.env;
    const now = dependencies.now?.() ?? new Date();
    const readFileText = dependencies.readFileText ?? ((path) => readFile(path, "utf8"));
    const writeFileText = dependencies.writeFileText ??
    ...
    if (!credentials) {
        return unavailableSnapshot(now, "Codex auth file is missing OAuth credentials. Re-run `codex login`.");
    }
    ...
    try {
        ...
        return unavailableSnapshot(now, "Unable to refresh Codex OAuth credentials. Re-run `codex login`.", "error");
    }
    try {
        ...
        if (!usageResponse.ok) {
            return unavailableSnapshot(now, `Codex usage request failed (${usageResponse.status}).`, "error");
        }
