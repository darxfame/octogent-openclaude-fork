...
import { DEFAULT_MONITOR_MAX_CACHE_AGE_MS, DEFAULT_MONITOR_MAX_POSTS, DEFAULT_MONITOR_SEARCH_WINDOW_DAYS, } from "./defaults";
const MONITOR_CONFIG_VERSION = 1;
const MONITOR_CACHE_VERSION = 1;
...
const normalizeMonitorPost = (value) => {
    ...
    const likeCount = typeof record.likeCount === "number" && Number.isFinite(record.likeCount)
        ? Math.max(0, Math.floor(record.likeCount))
        : null;
    const permalink = typeof record.permalink === "string" ? record.permalink : null;
    const matchedQueryTerm = typeof record.matchedQueryTerm === "string" ? record.matchedQueryTerm : null;
    if (source !== "x" ||
        id === null ||
        text === null ||
        author === null ||
...
const normalizeUsage = (value) => {
    ...
    const cap = typeof record.cap === "number" && Number.isFinite(record.cap) ? Math.max(0, record.cap) : null;
...
const normalizeConfig = (value) => {
    ...
    const maxCacheAgeMs = refreshPolicyRecord &&
    ...
    const maxPosts = refreshPolicyRecord &&
    ...
    const searchWindowDaysRaw = refreshPolicyRecord &&
        ...
        : null;
    const searchWindowDays = searchWindowDaysRaw !== null && VALID_MONITOR_SEARCH_WINDOW_DAYS.has(searchWindowDaysRaw)
        ? searchWindowDaysRaw
...
const writeJsonDocument = (path, value) => {
    mkdirSync(dirname(path), { recursive: true });
    writeFileSync(path, `${JSON.stringify(value, null, 2)}\n`, "utf8");
};
export const createFileMonitorRepository = (projectStateDir) => {
