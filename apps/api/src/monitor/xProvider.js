...
const normalizeOptionalSecret = (value) => {
    if (typeof value !== "string") {
        return null;
    }
...
const summarizeXCredentials = (credentials) => {
...
export const buildXRecentSearchQuery = (queryTerms) => {
...
const buildXApiUrl = (baseUrl, pathname, searchParams) => {
...
const asErrorMessage = (value) => {
    if (value instanceof Error) {
        return value.message;
    }
    return typeof value === "string" ? value : "Unknown error";
};
const truncateText = (value, maxLength = 220) => value.length > maxLength ? `${value.slice(0, maxLength)}...` : value;
const readResponseErrorDetail = async (response) => {
    try {
...
const assertXCredentials = (credentials) => {
    const parsed = toXCredentials(credentials);
    if (!parsed) {
...
const parseRecentSearchPayload = (payload, matchedQueryTerm) => {
    const record = asRecord(payload);
    if (!record) {
        return [];
    }
...
const fetchRecentSearchPage = async ({ fetchFn, baseUrl, credentials, query, startTime, nextToken, }) => {
...
const validateXCredentials = async ({ fetchFn, baseUrl, credentials, }) => {
    try {
        const now = new Date();
...
const fetchXRecentPosts = async ({ fetchFn, baseUrl, credentials, queryTerms, postLimit, searchWindowDays, now, }) => {
...
const fetchXUsage = async ({ fetchFn, baseUrl, usagePath, credentials, now, }) => {
    try {
...
export const createXMonitorProvider = ({ fetchFn = globalThis.fetch, apiBaseUrl = process.env.OCTOGENT_X_API_BASE_URL ?? DEFAULT_X_API_BASE_URL, usageEndpointPath = process.env.OCTOGENT_X_USAGE_ENDPOINT_PATH ?? DEFAULT_X_USAGE_ENDPOINT_PATH, } = {}) => ({
    providerId: "x",
    saveCredentials(input, now) {
...
//# sourceMappingURL=xProvider.js.map