...
import { DEFAULT_AGENT_PROVIDER, DEFAULT_TERMINAL_INACTIVITY_THRESHOLD_MS, TERMINAL_ID_PREFIX, TERMINAL_MAX_CONCURRENT_SESSIONS, } from "./terminalRuntime/constants";
import { conversationExportMarkdown, deleteAllConversations, deleteConversation, listConversationSessions, readConversationSession, searchConversations, } from "./terminalRuntime/conversations";
...
import { createTerminalRegistryPersistence, loadTerminalRegistry, pruneUiStateTerminalReferences, } from "./terminalRuntime/registry";
...
import { RuntimeInputError, } from "./terminalRuntime/types";
...
export const createTerminalRuntime = ({ workspaceCwd, projectStateDir, gitClient = createDefaultGitClient(), getApiBaseUrl = () => process.env.OCTOGENT_API_ORIGIN ?? "http://127.0.0.1:8787", maxConcurrentSessions, }) => {
    ...
    const terminalEventClients = new Set();
    ...
    const lifecycleStateToAgentState = (lifecycleState) => {
    ...
    const markTerminalRunning = (terminalId, { processId, startedAt }) => {
    ...
    const markTerminalEnded = (terminalId, details) => {
        const terminal = terminals.get(terminalId);
        if (!terminal) {
            return;
        }
    ...
    const resolveTerminalSession = (terminalId) => {
        const terminal = terminals.get(terminalId);
        if (terminal) {
            return {
    ...
    const broadcastTerminalStateChanged = (terminalId, agentRuntimeState, toolName) => {
        broadcastTerminalEvent({
    ...
    const channelMessaging = createChannelMessaging({
        terminals,
        sessions,
        writeInput: (terminalId, data) => sessionRuntime.writeInput(terminalId, data),
    });
    ...
    const allocateDefaultTerminalName = () => {
    ...
    const isTerminalRecentlyActive = (terminal) => {
        if (!terminal.lastActiveAt)
            return false;
        const thresholdMs = uiState.terminalInactivityThresholdMs ?? DEFAULT_TERMINAL_INACTIVITY_THRESHOLD_MS;
        return Date.now() - new Date(terminal.lastActiveAt).getTime() < thresholdMs;
    };
    const toTerminalSnapshot = (terminal) => {
    ...
    const broadcastTerminalEvent = (event) => {
    ...
    const collectTerminalCascade = (rootTerminalId) => {
    ...
    const createTerminal = ({ terminalId: requestedTerminalId, tentacleId: requestedTentacleId, worktreeId: requestedWorktreeId, tentacleName, workspaceMode = "shared", agentProvider, initialPrompt, initialInputDraft, baseRef, parentTerminalId, nameOrigin, autoRenamePromptContext, }) => {
        // Enforce max children per parent.
        if (parentTerminalId) {
            const childCount = [...terminals.values()].filter((t) => t.parentTerminalId === parentTerminalId).length;
            if (childCount >= MAX_CHILDREN_PER_PARENT) {
                throw new RuntimeInputError(`Parent terminal "${parentTerminalId}" already has ${MAX_CHILDREN_PER_PARENT} children (limit reached).`);
            }
        }
        const terminalId = requestedTerminalId && !terminals.has(requestedTerminalId)
            ? requestedTerminalId
            : allocateTerminalId();
        if (initialPrompt) {
            const capacity = sessionRuntime.getSessionCapacity();
            if (capacity.active >= capacity.max) {
                throw new RuntimeInputError(`Terminal session limit reached (${capacity.max}). Close an existing terminal session or increase OCTOGENT_MAX_TERMINAL_SESSIONS.`);
            }
        }
    ...
    return {
        ...
        readConversationSession(sessionId) {
            return readConversationSession(transcriptDirectoryPath, sessionId);
        },
        exportConversationSession(sessionId, format) {
        ...
        ...gitOps,
        createTerminal,
        renameTerminal(terminalId, tentacleName) {
            const terminal = terminals.get(terminalId);
            if (!terminal) {
                return null;
            }
        ...
        stopTerminal(terminalId) {
            ...
            if (!stoppedActiveSession && isProcessAlive(terminal.processId)) {
                try {
                    process.kill(terminal.processId, "SIGTERM");
                }
                catch {
        ...
        killTerminal(terminalId) {
            ...
            if (!killedActiveSession && isProcessAlive(terminal.processId)) {
                try {
                    process.kill(terminal.processId, signal);
                }
                catch {
                    // The lifecycle marker below still removes this terminal from the active set.
                }
        ...
        pruneTerminals() {
            const prunableStates = new Set(["stale", "exited", "stopped"]);
            const prunedTerminalIds = [];
            for (const terminal of terminals.values()) {
        ...
        deleteTerminal(terminalId) {
            ...
            for (const cascadeTerminalId of cascadeTerminalIds) {
                ...
                if (cascadeTerminal.workspaceMode === "worktree") {
                    worktreeManager.removeTentacleWorktree(cascadeTerminal.worktreeId ?? cascadeTerminal.tentacleId);
                }
        ...
        connectDirect(terminalId, listener) {
...
//# sourceMappingURL=terminalRuntime.js.map