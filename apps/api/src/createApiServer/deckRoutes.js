...
import { addTodoItem, createDeckTentacle, deleteDeckTentacle, deleteTodoItem, editTodoItem, listDeckAvailableSkills, parseTodoProgress, readDeckTentacles, readDeckVaultFile, toggleTodoItem, updateDeckTentacleSuggestedSkills, } from "../deck/readDeckTentacles";
import { resolvePrompt } from "../prompts";
import { MAX_CHILDREN_PER_PARENT, RuntimeInputError } from "../terminalRuntime";
import { readJsonBodyOrWriteError, writeJson, writeMethodNotAllowed, writeNoContent, writeText, } from "./routeHelpers";
...
const buildSingleTodoWorkerPrompt = async ({ promptsDir, workspaceCwd, tentacleId, tentacleName, todoItemText, terminalId, apiPort, }) => {
    ...
    return await resolvePrompt(promptsDir, "swarm-worker", {
        ...
        terminalId,
        apiPort,
        workspaceContextIntro: "You are working in the shared main workspace on the main branch, not in an isolated worktree.",
        workspaceGuidelines: [
        ...
        commitGuidance: "- Do NOT commit. Leave your completed changes uncommitted in the shared workspace and report what changed.",
        definitionOfDoneCommitStep: "Changes are left uncommitted in the shared main workspace, ready for operator review.",
        workspaceReminder: "Do not commit. Do not use worktrees.",
        parentTerminalId: "",
        parentSection: "",
    });
};
export const handleDeckTentaclesRoute = async ({ request, response, requestUrl, corsOrigin }, { workspaceCwd, projectStateDir }) => {
    if (requestUrl.pathname !== "/api/deck/tentacles")
        return false;
    ...
    if (request.method === "POST") {
        ...
        const suggestedSkills = body && Array.isArray(body.suggestedSkills)
            ? body.suggestedSkills.filter((skill) => typeof skill === "string")
            : [];
        ...
        const result = createDeckTentacle(workspaceCwd, { name, description, color, octopus, suggestedSkills }, projectStateDir);
        if (!result.ok) {
...
export const handleDeckSkillsRoute = async ({ request, response, requestUrl, corsOrigin }, { workspaceCwd }) => {
    if (requestUrl.pathname !== "/api/deck/skills")
        return false;
    if (request.method !== "GET") {
        writeMethodNotAllowed(response, corsOrigin);
        return true;
...
export const handleDeckTentacleItemRoute = async ({ request, response, requestUrl, corsOrigin }, { workspaceCwd, projectStateDir }) => {
...
export const handleDeckVaultFileRoute = async ({ request, response, requestUrl, corsOrigin }, { workspaceCwd }) => {
...
export const handleDeckTentacleSkillsRoute = async ({ request, response, requestUrl, corsOrigin }, { workspaceCwd, projectStateDir }) => {
    ...
    const suggestedSkills = Array.isArray(payload?.suggestedSkills)
        ? payload.suggestedSkills.filter((skill) => typeof skill === "string")
        : null;
    ...
    const tentacleId = decodeURIComponent(match[1]);
    const updated = updateDeckTentacleSuggestedSkills(workspaceCwd, tentacleId, suggestedSkills, projectStateDir);
    if (!updated) {
...
export const handleDeckTodoToggleRoute = async ({ request, response, requestUrl, corsOrigin }, { workspaceCwd }) => {
    ...
    if (!body.ok)
        return true;
    const { itemIndex, done } = body.payload;
...
export const handleDeckTodoEditRoute = async ({ request, response, requestUrl, corsOrigin }, { workspaceCwd }) => {
    ...
    if (!body.ok)
        return true;
    const { itemIndex, text } = body.payload;
...
export const handleDeckTodoAddRoute = async ({ request, response, requestUrl, corsOrigin }, { workspaceCwd }) => {
...
export const handleDeckTodoDeleteRoute = async ({ request, response, requestUrl, corsOrigin }, { workspaceCwd }) => {
...
export const handleDeckTodoSolveRoute = async ({ request, response, requestUrl, corsOrigin }, { runtime, workspaceCwd, projectStateDir, promptsDir, getApiPort }) => {
    ...
    if (existingTerminal) {
        writeJson(response, 409, { error: "A solve agent is already active for this todo item.", terminalId }, corsOrigin);
        return true;
    }
...
export const handleDeckTentacleSwarmRoute = async ({ request, response, requestUrl, corsOrigin }, { runtime, workspaceCwd, projectStateDir, promptsDir, getApiPort }) => {
    ...
    const workerWorkspaceMode = body.workspaceMode === undefined ? "worktree" : workspaceModeResult.workspaceMode;
    // Filter to specific item indices if requested.
    let targetItems = incompleteItems;
    if (Array.isArray(body.todoItemIndices)) {
        const requestedIndices = new Set(body.todoItemIndices.filter((v) => typeof v === "number"));
        targetItems = incompleteItems.filter((item) => requestedIndices.has(item.index));
        if (targetItems.length === 0) {
            writeJson(response, 400, { error: "None of the requested todo item indices are incomplete." }, corsOrigin);
            return true;
        }
    }
    ...
    if (existingSwarmIds.length > 0) {
        writeJson(response, 409, { error: "A swarm is already active for this tentacle.", existingSwarmIds }, corsOrigin);
        return true;
    }
    // Determine base ref: use tentacle's worktree branch if it exists, otherwise HEAD.
    const tentacleTerminal = existingTerminals.find((t) => t.tentacleId === tentacleId && t.workspaceMode === "worktree");
    ...
    const buildWorkerContextIntro = () => workerWorkspaceMode === "worktree"
    ...
    const buildWorkerGuidelines = (terminalId) => workerWorkspaceMode === "worktree"
    ...
    const buildWorkerCommitGuidance = () => workerWorkspaceMode === "worktree"
    ...
    const buildWorkerDefinitionOfDoneCommitStep = () => workerWorkspaceMode === "worktree"
        ? "Changes are committed with a descriptive message."
        : "Changes are left uncommitted in the shared workspace, ready for coordinator review.";
    const buildWorkerReminder = () => workerWorkspaceMode === "worktree" ? "Commit." : "Do not commit in shared mode.";
    const buildWorkerWorkspaceSection = () => workerWorkspaceMode === "worktree"
        ? [
            "Each worker commits to its own isolated branch:",
            "",
            ...workers.map((w) => `- \`octogent/${w.terminalId}\` — item #${w.todoIndex}: ${w.todoText}`),
        ].join("\n")
        : [
    ...
    const buildCompletionStrategySection = (baseBranch) => workerWorkspaceMode === "worktree"
        ? [
    ...
    try {
        ...
        if (needsParent && parentTerminalId) {
            ...
            const workerSpawnCommands = targetItems
                ...
                .join("\n");
            const parentBaseBranch = workerWorkspaceMode === "worktree" ? (baseRef === "HEAD" ? "main" : baseRef) : "main";
