...
import { handleConversationExportRoute, handleConversationItemRoute, handleConversationSearchRoute, handleConversationsCollectionRoute, } from "./conversationRoutes";
import { handleDeckSkillsRoute, handleDeckTentacleItemRoute, handleDeckTentacleSkillsRoute, handleDeckTentacleSwarmRoute, handleDeckTentaclesRoute, handleDeckTodoAddRoute, handleDeckTodoDeleteRoute, handleDeckTodoEditRoute, handleDeckTodoSolveRoute, handleDeckTodoToggleRoute, handleDeckVaultFileRoute, } from "./deckRoutes";
import { handleTentacleGitPullRequestRoute, handleTentacleGitRoute } from "./gitRoutes";
import { handleChannelMessagesRoute, handleHookRoute, handlePromptItemRoute, handlePromptsCollectionRoute, handleUiStateRoute, handleWorkspaceSetupRoute, } from "./miscRoutes";
import { handleMonitorConfigRoute, handleMonitorFeedRoute, handleMonitorRefreshRoute, } from "./monitorRoutes";
import { writeJson, writeNoContent } from "./routeHelpers";
import { getRequestCorsOrigin, isAllowedHostHeader, isAllowedOriginHeader, readHeaderValue, } from "./security";
import { handleTerminalActionRoute, handleTerminalItemRoute, handleTerminalPruneRoute, handleTerminalSnapshotsRoute, handleTerminalsCollectionRoute, } from "./terminalRoutes";
import { handleClaudeUsageRoute, handleCodexUsageRoute, handleGithubSummaryRoute, handleUsageHeatmapRoute, } from "./usageRoutes";
const MIME_TYPES = {
    ".html": "text/html",
...
const extractRoutePrefix = (pathname) => {
...
const logRequest = (method, path, status, startTime) => {
    logVerbose(`[API] ${method} ${path} ${status} ${Date.now() - startTime}ms`);
};
const serveStaticFile = async (response, webDistDir, pathname) => {
...
export const createApiRequestHandler = ({ runtime, workspaceCwd, projectStateDir, promptsDir, userPromptsDir, webDistDir, getApiBaseUrl, getApiPort, readClaudeUsageSnapshot, readClaudeOauthUsageSnapshot, readClaudeCliUsageSnapshot, readCodexUsageSnapshot, readGithubRepoSummary, scanUsageHeatmap, monitorService, invalidateClaudeUsageCache, codeIntelStore, allowRemoteAccess, }) => {
    ...
    return async (request, response) => {
        ...
        try {
            ...
            if (resolvedWebDistDir && request.method === "GET") {
                const served = (await serveStaticFile(response, resolvedWebDistDir, requestUrl.pathname)) ||
            ...
            console.error(`[API] Unhandled error: ${request.method ?? "?"} ${request.url ?? "/"}`, error instanceof Error ? (error.stack ?? error.message) : error);
