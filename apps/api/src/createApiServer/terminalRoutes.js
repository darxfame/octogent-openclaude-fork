...
import { parseTerminalAgentProvider, parseTerminalName, parseTerminalNameOrigin, parseTerminalWorkspaceMode, } from "./terminalParsers";
const buildTentacleInitialPrompt = (promptsDir, workspaceCwd, projectStateDir, tentacleId) => {
    const tentacle = readDeckTentacles(workspaceCwd, projectStateDir).find((entry) => entry.tentacleId === tentacleId);
    if (!tentacle) {
...
export const handleTerminalSnapshotsRoute = async ({ request, response, requestUrl, corsOrigin }, { runtime }) => {
...
export const handleTerminalsCollectionRoute = async ({ request, response, requestUrl, corsOrigin }, { runtime, workspaceCwd, projectStateDir, promptsDir, userPromptsDir, getApiPort }) => {
    ...
    try {
        ...
        const bodyPayload = bodyReadResult.payload;
        if (bodyPayload &&
        ...
        if (bodyPayload &&
            ...
            const templateVars = bodyPayload.promptVariables != null &&
                typeof bodyPayload.promptVariables === "object" &&
                !Array.isArray(bodyPayload.promptVariables)
                ? Object.fromEntries(Object.entries(bodyPayload.promptVariables)
            ...
            if (!templateVars.existingTerminals) {
                ...
                if (deckTentacles.length > 0) {
                    const listing = deckTentacles
                        .map((t) => `- **${t.displayName}** (\`${t.tentacleId}\`): ${t.description || "(no description)"}`)
                        .join("\n");
        ...
        if (!createTerminalInput.initialPrompt && createTerminalInput.tentacleId) {
            const defaultTentaclePrompt = await buildTentacleInitialPrompt(promptsDir, workspaceCwd, projectStateDir, createTerminalInput.tentacleId);
...
export const handleTerminalItemRoute = async ({ request, response, requestUrl, corsOrigin }, { runtime }) => {
...
export const handleTerminalActionRoute = async ({ request, response, requestUrl, corsOrigin }, { runtime }) => {
    ...
    const snapshot = action === "kill" ? runtime.killTerminal(terminalId) : runtime.stopTerminal(terminalId);
    if (!snapshot) {
        writeJson(response, 404, { error: "Terminal not found." }, corsOrigin);
        return true;
    }
...
export const handleTerminalPruneRoute = async ({ request, response, requestUrl, corsOrigin }, { runtime }) => {
