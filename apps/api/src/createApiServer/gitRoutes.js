...
import { parseTentacleCommitMessage, parseTentaclePullRequestCreateInput, parseTentacleSyncBaseRef, } from "./gitParsers";
...
export const handleTentacleGitRoute = async ({ request, response, requestUrl, corsOrigin }, { runtime }) => {
    ...
    try {
        ...
        if (action === "commit") {
            ...
            if (commitMessageResult.error || !commitMessageResult.message) {
                writeJson(response, 400, { error: commitMessageResult.error ?? "Commit message cannot be empty." }, corsOrigin);
                return true;
            }
...
export const handleTentacleGitPullRequestRoute = async ({ request, response, requestUrl, corsOrigin }, { runtime }) => {
    ...
    try {
        ...
        if (pullRequestInput.error || !pullRequestInput.title) {
            writeJson(response, 400, { error: pullRequestInput.error ?? "Pull request title cannot be empty." }, corsOrigin);
            return true;
        }
