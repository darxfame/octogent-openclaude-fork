...
import { ensureOctogentGitignoreEntry, ensureProjectScaffold, loadProjectConfig, loadProjectsRegistry, migrateStateToGlobal, registerProject, resolveEphemeralProjectStateDir, resolveProjectStateDir, } from "./projectPersistence";
import { clearRuntimeMetadata, readRuntimeMetadata, writeRuntimeMetadata } from "./runtimeMetadata";
import { collectStartupPrerequisiteReport, formatStartupPrerequisiteReport, } from "./startupPrerequisites";
...
const resolveRuntimeAssetPath = (...relativePathCandidates) => {
...
const initializeProject = (workspaceCwd, preferredName) => {
...
const resolveStartupProjectContext = (workspaceCwd) => {
...
const initProject = (name) => {
    ...
    console.log(`${created ? "Initialized" : "Updated"} Octogent project "${projectConfig.displayName}" at ${projectPath}`);
...
const canListenOnPort = (port) => new Promise((resolvePort) => {
...
const resolveRuntimeApiBase = () => {
    const explicitBase = process.env.OCTOGENT_API_ORIGIN?.trim() || process.env.OCTOGENT_API_BASE?.trim();
    if (explicitBase) {
        return explicitBase;
    }
...
const apiError = () => {
    console.error(`Error: Could not reach API at ${resolveRuntimeApiBase()}. Start Octogent in this project first.`);
    process.exit(1);
};
...
const startServer = async () => {
    ...
    const { isInitialized, projectDisplayName, projectStateDir } = resolveStartupProjectContext(workspaceCwd);
...
const tentacleList = async () => {
    ...
    try {
        ...
        const tentacles = (await response.json());
...
const terminalCreate = async () => {
    ...
    if (workspaceMode)
        body.workspaceMode = workspaceMode;
    if (terminalId)
        body.terminalId = terminalId;
    if (tentacleId)
        body.tentacleId = tentacleId;
    if (worktreeId)
        body.worktreeId = worktreeId;
    if (parentTerminalId)
        body.parentTerminalId = parentTerminalId;
    if (nameOrigin)
        body.nameOrigin = nameOrigin;
    if (autoRenamePromptContext)
        body.autoRenamePromptContext = autoRenamePromptContext;
    if (promptTemplate)
        body.promptTemplate = promptTemplate;
    if (promptVariables)
        body.promptVariables = promptVariables;
    try {
...
const terminalList = async () => {
    ...
    try {
        ...
        for (const terminal of terminals) {
            ...
            const pid = typeof terminal.processId === "number" && Number.isFinite(terminal.processId)
                ? ` pid=${terminal.processId}`
                : "";
            const reason = typeof terminal.lifecycleReason === "string" ? ` reason=${terminal.lifecycleReason}` : "";
...
const terminalAction = async (action) => {
    ...
    const apiBase = resolveRuntimeApiBase();
    try {
        const response = await fetch(`${apiBase}/api/terminals/${encodeURIComponent(terminalId)}/${action}`, {
            method: "POST",
            headers: { Accept: "application/json" },
        });
        const data = (await response.json());
...
const channelList = async () => {
    ...
    try {
        ...
        for (const message of messages) {
            ...
            console.log(`  [${message.messageId}] from=${message.fromTerminalId || "(unknown)"} status=${status}: ${message.content}`);
        }
    }
    catch {
        apiError();
