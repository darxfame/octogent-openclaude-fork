...
class FakeGitClient {
    worktreeStatusByCwd = new Map();
    commitsByCwd = new Map();
    pushesByCwd = new Map();
    syncsByCwd = new Map();
    pullRequestByCwd = new Map();
    worktrees = new Map();
    branches = new Set();
    ...
    setRepositoryAvailable(available) {
        this.repositoryAvailable = available;
    }
    setFailRemoveWorktree(shouldFail) {
        this.failRemoveWorktree = shouldFail;
    }
    setFailCommit(shouldFail) {
        this.failCommit = shouldFail;
    }
    setFailPush(shouldFail) {
        this.failPush = shouldFail;
    }
    setFailSync(shouldFail) {
        this.failSync = shouldFail;
    }
    setFailCreatePullRequest(shouldFail) {
        this.failCreatePullRequest = shouldFail;
    }
    setFailMergePullRequest(shouldFail) {
        this.failMergePullRequest = shouldFail;
    }
    setWorktreeStatus(cwd, status) {
    ...
    setWorktreePullRequest(cwd, pullRequest) {
    ...
    createPullRequest({ cwd, title, baseRef, headRef, }) {
    ...
    mergeCurrentBranchPullRequest({ cwd, }) {
...
describe("createApiServer", () => {
    ...
    const toWebSocketBaseUrl = (httpBaseUrl) => httpBaseUrl.startsWith("https://")
        ? httpBaseUrl.replace("https://", "wss://")
        : httpBaseUrl.replace("http://", "ws://");
    const waitForRegistryDocument = async (workspaceCwd, predicate) => {
    ...
    const writeConversationTranscript = (workspaceCwd, sessionId, events) => {
    ...
    const writeClaudeTurns = (workspaceCwd, sessionId, turns) => {
        const transcriptDirectory = join(workspaceCwd, ".octogent", "state", "transcripts");
        mkdirSync(transcriptDirectory, { recursive: true });
    ...
    it("returns assembled conversation details and export payloads", async () => {
        ...
        const jsonExportResponse = await fetch(`${baseUrl}/api/conversations/terminal-2-agent-1/export?format=json`, {
            method: "GET",
            headers: {
                Accept: "application/json",
            },
        ...
        const markdownExportResponse = await fetch(`${baseUrl}/api/conversations/terminal-2-agent-1/export?format=md`, {
            method: "GET",
        });
    ...
    it("returns 400 for unsupported conversation export format", async () => {
        ...
        const response = await fetch(`${baseUrl}/api/conversations/terminal-3-agent-1/export?format=txt`, {
            method: "GET",
            headers: {
                Accept: "application/json",
            },
    ...
    it("POST /api/hooks/user-prompt-submit auto-renames generated default terminal names", async () => {
        ...
        const hookResponse = await fetch(`${baseUrl}/api/hooks/user-prompt-submit?octogent_session=terminal-1`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ prompt: "Investigate flaky CI failures" }),
        });
        expect(hookResponse.status).toBe(200);
        const secondHookResponse = await fetch(`${baseUrl}/api/hooks/user-prompt-submit?octogent_session=terminal-1`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
    ...
    it("reports file-backed workspace setup status and updates it through setup actions", async () => {
        ...
        expect(initialPayload.steps).toEqual(expect.arrayContaining([
        ...
        expect(finalPayload.steps).toEqual(expect.arrayContaining([
    ...
    it("ignores a root project skills SKILL.md file and only lists folder-based skills", async () => {
        ...
        writeFileSync(join(skillsDir, "docs-writer", "SKILL.md"), [
            "---",
            "name: docs-writer",
            "description: Helps keep docs aligned with product changes.",
            "---",
    ...
    it("creates tentacles with suggested skills and appends the managed context block", async () => {
        ...
        const context = readFileSync(join(workspaceCwd, ".octogent", "tentacles", "docs", "CONTEXT.md"), "utf8");
    ...
    it("updates tentacle suggested skills and removes the managed context block when cleared", async () => {
        ...
        await expect(clearResponse.json()).resolves.toEqual(expect.objectContaining({
            tentacleId: "docs",
            suggestedSkills: [],
        }));
    ...
    it("creates new tentacles with unique incremental ids", async () => {
        ...
        await expect(createFirstResponse.json()).resolves.toEqual(expect.objectContaining({
            terminalId: "terminal-1",
            label: "terminal-1",
            state: "live",
            tentacleId: "terminal-1",
        ...
        await expect(createSecondResponse.json()).resolves.toEqual(expect.objectContaining({
    ...
    it("reuses the minimum available tentacle number after deletions", async () => {
        ...
        await expect(createThirdResponse.json()).resolves.toEqual(expect.objectContaining({
            tentacleId: "terminal-1",
        }));
    });
    ...
    it("persists tentacle metadata without runtime bootstrap flags", async () => {
        ...
        expect(createResponse.status).toBe(201);
        const registryDocument = await waitForRegistryDocument(workspaceCwd, (document) => document.terminals.some((terminal) => terminal.terminalId === "terminal-1" &&
            terminal.tentacleId === "terminal-1" &&
            terminal.workspaceMode === "shared"));
        expect(registryDocument.terminals).toEqual(expect.arrayContaining([
            expect.objectContaining({
                terminalId: "terminal-1",
                tentacleId: "terminal-1",
                workspaceMode: "shared",
    ...
    it("marks auto-started prompted terminals as active immediately", async () => {
        ...
        await expect(snapshotsResponse.json()).resolves.toEqual(expect.arrayContaining([
            expect.objectContaining({
                terminalId: "terminal-1",
                hasUserPrompt: true,
            }),
    ...
    it("creates isolated worktree terminals with dedicated cwd", async () => {
        ...
        expect(gitClient.getWorktree(expectedWorktreePath)).toEqual(expect.objectContaining({
            cwd: workspaceCwd,
    ...
    it("refreshes builtin prompts from promptsDir on server start", async () => {
        ...
        writeFileSync(join(projectStateDir, "prompts", "core", "swarm-parent.md"), "stale prompt with {{workerBranches}}\n", "utf8");
        writeFileSync(join(promptsDir, "swarm-parent.md"), "fresh prompt with {{workerSpawnCommands}}\n", "utf8");
        const baseUrl = await startServer({
            workspaceCwd,
            projectStateDir,
            promptsDir,
    ...
    it("spawns a shared-workspace todo agent for an individual item", async () => {
        ...
        writeFileSync(join(workspaceCwd, ".octogent", "tentacles", "docs-knowledge", "CONTEXT.md"), "# Docs & Knowledge\n", "utf8");
        writeFileSync(join(workspaceCwd, ".octogent", "tentacles", "docs-knowledge", "todo.md"), "# Todo\n\n- [ ] Audit docs\n- [ ] Consolidate principles\n", "utf8");
        const baseUrl = await startServer({ workspaceCwd });
    ...
    it("auto-renames todo agents from the todo item context on first prompt submit", async () => {
        ...
        expect(solveResponse.status).toBe(201);
        const hookResponse = await fetch(`${baseUrl}/api/hooks/user-prompt-submit?octogent_session=docs-knowledge-todo-0`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
    ...
    it("limits swarm prompts to the top-priority items that fit under the child cap", async () => {
        ...
        writeFileSync(join(workspaceCwd, ".octogent", "tentacles", "docs-knowledge", "CONTEXT.md"), "# Docs & Knowledge\n", "utf8");
        const todoItems = Array.from({ length: MAX_CHILDREN_PER_PARENT + 4 }, (_, index) => `- [ ] item ${index}`).join("\n");
        writeFileSync(join(workspaceCwd, ".octogent", "tentacles", "docs-knowledge", "todo.md"), `# Todo\n\n${todoItems}\n`, "utf8");
        const baseUrl = await startServer({ workspaceCwd });
        ...
        const promptTemplate = readFileSync(join(process.cwd(), "..", "..", "prompts", "swarm-parent.md"), "utf8");
        expect(promptTemplate).toContain("Treat the listed workers as the highest-priority items and proceed without asking the user whether to batch, reprioritize, or raise the limit.");
    });
    ...
    it("stops and prunes stale terminal records through lifecycle endpoints", async () => {
        ...
        await expect(stopResponse.json()).resolves.toEqual(expect.objectContaining({
            terminalId: "terminal-1",
...
//# sourceMappingURL=createApiServer.test.js.map