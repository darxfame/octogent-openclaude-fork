...
import { invalidateUsageCache, parseCliUsageOutput, readClaudeUsageSnapshot, resetCliSession, stripAnsiCodes, } from "../src/claudeUsage";
...
describe("readClaudeUsageSnapshot", () => {
    ...
    it("falls back to OAuth when CLI returns null", async () => {
        const fetchMock = vi.fn().mockResolvedValueOnce(new Response(usageResponseBody, {
            status: 200,
    ...
    it("serves the last successful oauth snapshot when a later oauth request is rate limited", async () => {
        ...
        const okSnapshot = await readClaudeUsageSnapshot({
            ...deps,
            fetchImpl: async () => new Response(usageResponseBody, {
                status: 200,
                headers: { "Content-Type": "application/json" },
            }),
        });
    ...
    it("serves a persisted snapshot immediately and refreshes in background", async () => {
        ...
        resetCliSession();
        const fetchMock = vi.fn().mockImplementation(async () => await new Promise((resolve) => {
            setTimeout(() => {
                resolve(new Response(usageResponseBody, {
                    status: 200,
                    headers: { "Content-Type": "application/json" },
                }));
            }, 50);
...
//# sourceMappingURL=claudeUsage.test.js.map