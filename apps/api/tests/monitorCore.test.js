...
describe("monitor core logic", () => {
    ...
    it("throws when no valid query terms are provided", () => {
        expect(() => buildXRecentSearchQuery(["", "   "])).toThrow("At least one X query term is required.");
    });
    ...
    it("dedupes, ranks by like count desc, and limits to top 30", () => {
        const basePosts = Array.from({ length: 32 }).map((_, index) => ({
            source: "x",
            id: `id-${index + 1}`,
            text: `Post ${index + 1}`,
...
//# sourceMappingURL=monitorCore.test.js.map