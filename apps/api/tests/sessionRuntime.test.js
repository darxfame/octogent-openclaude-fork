...
const createUpgradeRequest = (tentacleId) => ({
    url: `/api/terminals/${tentacleId}/ws`,
});
const parseSentMessages = (socket) => socket.sentMessages.map((raw) => JSON.parse(raw));
describe("createSessionRuntime", () => {
    ...
    it("keeps a session alive across reconnects and replays scrollback history", () => {
        ...
        expect(runtime.handleUpgrade(createUpgradeRequest(tentacleId), {}, Buffer.alloc(0))).toBe(true);
    ...
    it("writes normalized transcript events for each terminal session", async () => {
        ...
        expect(transcriptEvents.some((event) => event.type === "session_end" && event.reason === "session_close")).toBe(true);
    });
    ...
    it("reports session lifecycle start and exit through callbacks", () => {
        ...
        websocketServer.nextSocket = socket;
        expect(runtime.handleUpgrade(createUpgradeRequest(tentacleId), {}, Buffer.alloc(0))).toBe(true);
        expect(onSessionStart).toHaveBeenCalledWith(tentacleId, expect.objectContaining({
            processId: 3210,
            startedAt: expect.any(String),
        }));
        pty.emit("exit", { exitCode: 7, signal: 0 });
        expect(onSessionEnd).toHaveBeenCalledWith(tentacleId, expect.objectContaining({
            reason: "pty_exit",
            exitCode: 7,
            signal: 0,
...
//# sourceMappingURL=sessionRuntime.test.js.map