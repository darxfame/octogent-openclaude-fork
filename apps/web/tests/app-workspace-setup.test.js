...
const buildSetupSnapshot = (overrides = {}) => ({
    isFirstRun: true,
    ...
    steps: [
        ...
        },
        {
            ...
            guidance: "Git ignore entry is missing. Create or update .gitignore with the Octogent workspace path.",
...
const mockAppRequests = (resolveSetup, options = {}) => {
    return vi.spyOn(globalThis, "fetch").mockImplementation(async (input, init) => {
        ...
        if (url.endsWith("/api/setup/steps/ensure-gitignore") && method === "POST") {
            return jsonResponse(options.onEnsureGitignoreStep ? options.onEnsureGitignoreStep() : resolveSetup());
        }
...
describe("App workspace setup", () => {
    ...
    it("only marks a setup step complete after the refreshed server snapshot says it is done", async () => {
        ...
        mockAppRequests(() => currentSetup, {
            onEnsureGitignoreStep: () => {
                currentSetup = buildSetupSnapshot({
                    steps: buildSetupSnapshot().steps.map((step) => step.id === "ensure-gitignore"
                        ? {
                            ...step,
                            complete: true,
        ...
        await waitFor(() => {
            ...
            expect(within(gitignoreStep).getByText("Done")).toBeInTheDocument();
        });
    });
});
//# sourceMappingURL=app-workspace-setup.test.js.map