...
vi.mock("../src/components/canvas/SessionNode", () => ({
    SessionNode: ({ node, onClick, }) => (<button type="button" data-node-id={node.id} onClick={() => onClick(node.id)}>
      {node.label}
    </button>),
}));
vi.mock("../src/components/canvas/OctopusNode", () => ({
    OctopusNode: ({ node, onClick, }) => (<g data-node-id={node.id} onClick={() => onClick(node.id)} onKeyDown={(event) => {
...
vi.mock("../src/components/canvas/CanvasTerminalColumn", () => ({
    CanvasTerminalColumn: ({ node, panelRef, onMinimize, onClose, }) => (<section ref={panelRef} data-testid={`panel-${node.id}`} tabIndex={-1}>
...
describe("CanvasPrimaryView", () => {
    ...
    it("reveals and focuses a newly opened terminal panel when a session node is clicked", async () => {
        ...
        if (!terminalButton)
            throw new Error("Missing terminal button");
    ...
    it("minimizes a terminal panel separately from closing the terminal session", async () => {
        ...
        render(<CanvasPrimaryView columns={[
                {
            ...
            ]} isUiStateHydrated onCloseActiveSession={onCloseActiveSession}/>);
        const [terminalButton] = screen.getAllByRole("button", { name: "terminal-1" });
        expect(terminalButton).toBeDefined();
        if (!terminalButton)
            throw new Error("Missing terminal button");
    ...
    it("auto-opens a newly created child terminal when its parent panel is already open", async () => {
        const { rerender } = render(<CanvasPrimaryView columns={[
                {
                    terminalId: "terminal-1",
                    label: "terminal-1",
                    state: "live",
        ...
        rerender(<CanvasPrimaryView columns={[
                {
                    terminalId: "terminal-1",
                    label: "terminal-1",
                    state: "live",
            ...
            ]} isUiStateHydrated recentlyCreatedTerminal={{
                terminalId: "terminal-2",
                label: "terminal-2",
                state: "live",
                tentacleId: "tentacle-a",
    ...
    it("updates an open terminal panel label when the terminal is renamed", async () => {
        ...
        fireEvent.click(terminalButton);
        await waitFor(() => {
            expect(screen.getByTestId("panel-a:terminal-1")).toHaveTextContent("panel a:terminal-1 label tentacle-a");
        });
        rerender(<CanvasPrimaryView columns={[
                {
                    terminalId: "terminal-1",
        ...
        await waitFor(() => {
            expect(screen.getByTestId("panel-a:terminal-1")).toHaveTextContent("panel a:terminal-1 label renamed-tentacle");
        });
    });
    it("shows tentacle maintenance actions in the context menu and passes the tentacle ID", async () => {
        ...
        const { container } = render(<CanvasPrimaryView columns={[]} isUiStateHydrated onTentacleAction={onTentacleAction}/>);
        const tentacleNode = container.querySelector('[data-node-id="t:tentacle-a"]');
        expect(tentacleNode).not.toBeNull();
        fireEvent.contextMenu(tentacleNode, { clientX: 160, clientY: 120 });
...
//# sourceMappingURL=CanvasPrimaryView.test.js.map