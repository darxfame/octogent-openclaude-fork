...
describe("App Monitor runtime", () => {
    ...
    it("saves X credentials, renders monitor feed rows, and supports manual refresh", async () => {
        ...
        const monitorConfigPatchBodies = [];
        let refreshCount = 0;
        vi.spyOn(globalThis, "fetch").mockImplementation(async (input, init) => {
            ...
            if (url.endsWith("/api/monitor/config") && method === "PATCH") {
                if (typeof init?.body === "string") {
                    monitorConfigPatchBodies.push(JSON.parse(init.body));
                }
                return jsonResponse({
                    providerId: "x",
                    queryTerms: ["Codex"],
        ...
        expect(within(monitorView).getByRole("button", { name: "Resources" })).toHaveAttribute("aria-current", "page");
        fireEvent.click(within(monitorView).getByRole("button", { name: "Configure" }));
        expect(within(monitorView).getByRole("button", { name: "Configure" })).toHaveAttribute("aria-current", "page");
        ...
        await waitFor(() => {
            expect(monitorConfigPatchBodies.some((body) => Array.isArray(body.queryTerms) ? body.queryTerms.includes("Agent Ops") : false)).toBe(true);
        });
        expect(monitorConfigPatchBodies.some((body) => typeof body.refreshPolicy === "object" &&
            body.refreshPolicy !== null &&
            body.refreshPolicy.searchWindowDays === 3)).toBe(true);
    ...
    it("hydrates the bottom telemetry tape after reload without opening the monitor view", async () => {
        ...
        await waitFor(() => {
            expect(within(telemetryTape).getAllByText("@octogent")).toHaveLength(2);
            expect(within(telemetryTape).queryByText("Waiting for X resources...")).not.toBeInTheDocument();
        });
    });
...
//# sourceMappingURL=app-monitor-runtime.test.js.map