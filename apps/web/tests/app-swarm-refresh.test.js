...
vi.mock("../src/app/hooks/useInitialColumnsHydration", async () => {
    ...
    return {
        useInitialColumnsHydration: ({ setColumns, setIsLoading, setIsUiStateHydrated, }) => {
            React.useEffect(() => {
                setColumns([]);
                setIsLoading(false);
...
vi.mock("../src/components/PrimaryViewRouter", () => ({
    PrimaryViewRouter: ({ canvasPrimaryViewProps, }) => (<div>
      <button type="button" onClick={() => void canvasPrimaryViewProps.onSpawnSwarm?.("docs-knowledge", "shared")}>
        Spawn Swarm
      </button>
      <span>
...
describe("App swarm actions", () => {
    ...
    it("does not force a terminal snapshot refresh after spawning a swarm", async () => {
        ...
        await waitFor(() => {
            expect(fetchMock.mock.calls.some(([calledUrl, calledInit]) => String(calledUrl).endsWith("/api/deck/tentacles/docs-knowledge/swarm") &&
                (calledInit?.method ?? "GET") === "POST")).toBe(true);
        });
        expect(terminalSnapshotReads).toBe(0);
    });
});
//# sourceMappingURL=app-swarm-refresh.test.js.map