...
describe("RuntimeStatusStrip", () => {
    ...
    it("uses a 5h label for oauth-backed usage", () => {
        render(<RuntimeStatusStrip sparklinePoints="" usageData={null} claudeUsage={{
                status: "ok",
                source: "oauth-api",
    ...
    it("marks the refresh button as rotating while Claude usage is refreshing", () => {
        render(<RuntimeStatusStrip sparklinePoints="" usageData={null} claudeUsage={null} isRefreshingClaudeUsage onRefreshClaudeUsage={() => { }}/>);
        expect(screen.getByRole("button", { name: "Refresh Claude usage" })).toHaveAttribute("data-refreshing", "true");
    });
});
//# sourceMappingURL=RuntimeStatusStrip.test.js.map