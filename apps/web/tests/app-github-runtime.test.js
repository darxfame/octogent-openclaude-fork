...
const buildRecentCommits = () => Array.from({ length: 50 }, (_, index) => {
...
describe("App GitHub runtime views", () => {
    ...
    it("renders the Activity view with the GitHub overview graph", async () => {
        ...
        expect(within(githubView).getByRole("button", { name: "Refresh GitHub overview data" })).toBeInTheDocument();
        ...
        const graphPoint = container.querySelector(".github-overview-graph-point[aria-label='2026-02-27 · 8 commits']");
        expect(graphPoint).not.toBeNull();
        expect(container.querySelectorAll(".github-overview-graph-point")).toHaveLength(3);
        fireEvent.mouseEnter(graphPoint);
        ...
        const graphSvg = container.querySelector(".github-overview-graph-surface svg");
...
//# sourceMappingURL=app-github-runtime.test.js.map