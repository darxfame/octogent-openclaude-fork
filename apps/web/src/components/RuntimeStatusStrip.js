...
const UsageRail = ({ label, percent, loading, title, }) => {
    ...
    return (<div className="console-status-usage-row" data-has-tooltip={title ? "true" : undefined} tabIndex={title ? 0 : -1} onMouseEnter={(event) => showTooltip(event.clientX, event.clientY)} onMouseMove={(event) => showTooltip(event.clientX, event.clientY)} onMouseLeave={() => setTooltip(null)} onBlur={() => setTooltip(null)} onFocus={(event) => {
            if (!title)
                return;
      ...
      <span className="console-status-usage-rail">
        <span className="console-status-usage-rail-fill" style={{ width: `${Math.min(100, percent ?? 0)}%` }}/>
      </span>
      {title && tooltip ? (<span className="console-status-usage-tooltip" style={{
...
export const RuntimeStatusStrip = ({ sparklinePoints, usageData, claudeUsage, isRefreshingClaudeUsage = false, onRefreshClaudeUsage, }) => {
    ...
    return (<section className="console-status-strip" aria-label="Runtime status strip">
      <div className="console-status-main">
        <OctopusGlyph className="console-status-octopus-icon" animation="sway" expression="normal" scale={2}/>
          ...
          {usageBars.length > 0 ? (<>
              <div className="console-status-usage-mini-chart">
                <svg viewBox={`0 0 ${MINI_USAGE_WIDTH} ${MINI_USAGE_HEIGHT}`} role="presentation">
                  {usageBars.map((bar, index) => (<rect key={`${index}-${bar.x}-${bar.height}`} x={bar.x} y={bar.y} width={bar.width} height={bar.height} rx={0.5}/>))}
                </svg>
              </div>
      ...
      <div className="console-status-claude-usage" aria-label="Claude usage limits">
        {onRefreshClaudeUsage && (<button type="button" className="console-status-claude-usage-refresh" onClick={onRefreshClaudeUsage} aria-label="Refresh Claude usage" title="Refresh Claude usage" data-refreshing={showRefreshSpin ? "true" : "false"}>
            ↻
          </button>)}
        ...
        <div className="console-status-claude-usage-bars">
          <UsageRail label={claudeUsageState.label} percent={claudeUsageState.sessionPercent} loading={claudeUsageState.loading} {...(claudeUsageState.message ? { title: claudeUsageState.message } : {})}/>
          <UsageRail label="Week (all)" percent={claudeUsageState.weekPercent} loading={claudeUsageState.loading} {...(claudeUsageState.message ? { title: claudeUsageState.message } : {})}/>
        </div>
      </div>
    </section>);
};
//# sourceMappingURL=RuntimeStatusStrip.js.map