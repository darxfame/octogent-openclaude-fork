...
import { GitBranch, Hexagon, Layers, ListTodo, Maximize, Pause, Play, RefreshCw, Sparkles, Terminal as TerminalIcon, Trash2, } from "lucide-react";
...
const buildTentacleNodeId = (tentacleId) => `t:${tentacleId}`;
...
const renderEdgeActivityDots = (path, color, keyPrefix) => [0, 1, 2].flatMap((index) => [
    <circle key={`${keyPrefix}-trail-${index}`} className="canvas-edge-activity-dot canvas-edge-activity-dot--trail" r={4.6} fill={color} opacity={Math.max(0.14, 0.28 - index * 0.04)}>
      <animateMotion path={path} begin={`${index * 0.62}s`} dur="1.9s" repeatCount="indefinite" rotate="auto"/>
      <animate attributeName="r" values="3.8;5.2;3.8" dur="1.9s" begin={`${index * 0.62}s`} repeatCount="indefinite"/>
    </circle>,
    <circle key={`${keyPrefix}-dot-${index}`} className="canvas-edge-activity-dot" r={3.2} fill="#fff4cc" stroke={color} strokeWidth={1.2} opacity={Math.max(0.7, 1 - index * 0.08)}>
      <animateMotion path={path} begin={`${index * 0.62}s`} dur="1.9s" repeatCount="indefinite" rotate="auto"/>
      <animate attributeName="r" values="2.8;3.8;2.8" dur="1.9s" begin={`${index * 0.62}s`} repeatCount="indefinite"/>
    </circle>,
]);
export const CanvasPrimaryView = ({ columns, runtimeStateStore: providedRuntimeStateStore, isUiStateHydrated, canvasOpenTerminalIds, canvasOpenTentacleIds, canvasTerminalsPanelWidth: persistedTerminalsPanelWidth, workspaceSetup = null, isWorkspaceSetupLoading = false, workspaceSetupError = null, runningWorkspaceSetupStepId = null, onRunWorkspaceSetupStep, onLaunchWorkspaceSetupPlanner, recentlyCreatedTerminal, onCanvasOpenTerminalIdsChange, onCanvasOpenTentacleIdsChange, onCanvasTerminalsPanelWidthChange, onCreateAgent, onCreateTerminal, onCreateWorktreeTerminal, onCreateTentacle, onSpawnSwarm, onSolveTodoItem, onOctobossAction, onTentacleAction, onNavigateToConversation, onCloseActiveSession, onDeleteActiveSession, pendingDeleteTerminal, isDeletingTerminalId, onCancelDelete, onConfirmDelete, onTerminalRenamed, onTerminalActivity, onRefreshColumns, }) => {
    ...
    const [openTentacles, setOpenTentacles] = useState(new Map());
    ...
    const { nodes, edges, tentacleById, sessionsByTentacleId, refresh: refreshGraphData, refreshDeckTentacles, } = useCanvasGraphData({ columns, enabled: true, agentRuntimeStates });
    ...
    useEffect(() => {
        if (hasHydratedTentacles.current)
            return;
        if (!isUiStateHydrated)
            return;
        if (!hasTentacleNodes)
    ...
    const handleCloseTentacle = useCallback((nodeId) => {
        setOpenTentacles((prev) => {
            const next = new Map(prev);
            next.delete(nodeId);
            return next;
    ...
    const handleCloseTerminal = useCallback((node) => {
        ...
        const terminal = columns.find((entry) => entry.terminalId === node.sessionId);
        onCloseActiveSession?.(node.sessionId, terminal?.tentacleName ?? node.label, terminal?.workspaceMode ?? node.workspaceMode);
    ...
    useEffect(() => {
        if (!isHydratingTerminals && openTerminalCount === 0 && openTentacleCount === 0)
            return;
    ...
    const handleCreateAgent = useCallback((tentacleId) => {
        if (!onCreateAgent)
            return;
        setContextMenu(null);
    ...
    const handleSpawnSwarm = useCallback((tentacleId, workspaceMode) => {
        setContextMenu(null);
        void onSpawnSwarm?.(tentacleId, workspaceMode);
    }, [onSpawnSwarm]);
    const handleOctobossAction = useCallback((action) => {
        setContextMenu(null);
    ...
    const handleTentacleAction = useCallback((tentacleId, action) => {
        setContextMenu(null);
    ...
    const tentacleNodes = simulatedNodes.filter((n) => n.type === "tentacle" || n.type === "octoboss");
    const sessionNodes = simulatedNodes.filter((n) => {
        if (n.type === "tentacle" || n.type === "octoboss")
            return false;
    ...
    return (<section ref={containerRef} className="canvas-view" aria-label="Canvas graph view">
      <div className={`canvas-graph-panel${hasPanels ? " canvas-graph-panel--split" : ""}`}>
        <svg aria-label="Canvas graph" ref={svgRef} className={`canvas-svg${isPanning || dragNodeId ? " canvas-svg--panning" : ""}`} onWheel={handleWheel} onPointerDown={handleCanvasPointerDown} onPointerMove={handleSvgPointerMove} onPointerUp={handleSvgPointerUp} onClick={handleSvgClick} onKeyDown={(e) => {
            if (e.key === "Escape") {
                e.preventDefault();
                setContextMenu(null);
                setSelectedNodeId(null);
          ...
          <g transform={`translate(${transform.translateX}, ${transform.translateY}) scale(${transform.scale})`}>
            {Array.from(sessionEdgesBySource.entries()).flatMap(([sourceId, group]) => group.map(({ source, target }, index) => {
            ...
            return (<g key={`${sourceId}->${target.id}`}>
                    <path className="canvas-edge" d={path} fill="none" stroke={active ? (selectedColor ?? source.color) : "#C0C0C0"} strokeWidth={active ? 2 : 1.5} strokeOpacity={1}/>
            ...
            const selectedColor = selectedNodeId
                ? (nodesById.get(selectedNodeId)?.color ?? null)
                : null;
            return (<OctopusNode key={node.id} node={node} connectedNodes={connected} isSelected={selectedNodeId === node.id} selectedNodeId={selectedNodeId} selectedNodeColor={selectedColor} onPointerDown={handleNodePointerDown} onClick={handleNodeClick}/>);
        })}

            {/* Render session nodes on top */}
            {sessionNodes.map((node) => (<SessionNode key={node.id} node={node} isSelected={selectedNodeId === node.id} onPointerDown={handleNodePointerDown} onClick={handleNodeClick}/>))}
          </g>
        </svg>

        {/* Canvas toolbar — top-left action buttons */}
        <div className="canvas-toolbar" role="toolbar" aria-label="Canvas actions">
          <button type="button" className="canvas-toolbar-btn" onClick={() => {
          ...
          <div className="canvas-toolbar-separator"/>
          <button type="button" className={`canvas-toolbar-btn${hideIdleTerminals ? " canvas-toolbar-btn--active" : ""}`} onClick={() => setHideIdleTerminals((prev) => !prev)}>
            <span className="canvas-toolbar-icon">
          ...
          <div className="canvas-toolbar-separator"/>
          <button type="button" className="canvas-toolbar-btn canvas-toolbar-btn--danger" onClick={() => setIsDeleteAllDialogOpen(true)}>
            <span className="canvas-toolbar-icon">
              <Trash2 size={14}/>
            </span>
        ...
        {waitingNodes.length > 0 && (<div className="canvas-waiting-list">
            {waitingNodes.map((node) => {
                ...
                const prefix = node.agentRuntimeState === "waiting_for_permission"
                    ...
                    : "Waiting: ";
                return (<button key={node.id} type="button" className="canvas-waiting-bar" onClick={() => handleNodeClick(node.id)}>
        ...
        {shouldShowWorkspaceSetupCard && (<div className="canvas-setup-overlay">
            <WorkspaceSetupCard workspaceSetup={workspaceSetup} isLoading={isWorkspaceSetupLoading} error={workspaceSetupError} onRunStep={(stepId) => {
                void onRunWorkspaceSetupStep?.(stepId);
            }} onLaunchClaudeCode={() => {
                void handleLaunchWorkspaceSetupPlanner();
            }} isLaunchingAgent={isLaunchingWorkspaceSetupPlanner} isRunningStepId={runningWorkspaceSetupStepId}/>
          </div>)}
      </div>

      {hasPanels && (<>
          <div className="canvas-panel-divider" role="separator" aria-orientation="vertical" tabIndex={0} onPointerDown={handleDividerPointerDown} onPointerMove={handleDividerPointerMove} onPointerUp={handleDividerPointerUp}/>
          <div ref={terminalsPanelRef} className="canvas-terminals-panel" style={terminalsPanelWidth != null ? { flex: `0 0 ${terminalsPanelWidth}px` } : undefined}>
            {Array.from(openTentacles.entries()).map(([nodeId, node]) => (<CanvasTentaclePanel key={nodeId} node={node} isFocused={selectedNodeId === nodeId} panelRef={setPanelRef(nodeId)} tentacle={tentacleById.get(node.tentacleId) ?? null} sessions={sessionsByTentacleId.get(node.tentacleId) ?? []} onClose={() => handleCloseTentacle(nodeId)} onFocus={() => setSelectedNodeId(nodeId)} onCreateAgent={(tentacleId) => {
                ...
                }} onNavigateToConversation={onNavigateToConversation} onRefreshTentacleData={refreshDeckTentacles}/>))}
            {isHydratingTerminals && openTerminals.size === 0 && (<div className="canvas-terminal-skeleton">
            ...
            {Array.from(openTerminals.entries()).map(([nodeId, node]) => (<CanvasTerminalColumn key={nodeId} node={node} terminals={columns} layoutVersion={terminalLayoutVersion} isFocused={selectedNodeId === nodeId} panelRef={setPanelRef(nodeId)} onMinimize={() => handleMinimizeTerminal(nodeId)} onClose={() => handleCloseTerminal(node)} onFocus={() => setSelectedNodeId(nodeId)} onTerminalRenamed={onTerminalRenamed} onTerminalActivity={onTerminalActivity}/>))}
          </div>
        </>)}

      {/* Context menu */}
      {contextMenu && (<>
          <div aria-label="Close canvas context menu" className="canvas-context-menu-backdrop" onClick={() => setContextMenu(null)} onContextMenu={(e) => {
                e.preventDefault();
                e.stopPropagation();
          ...
          <div className="canvas-context-menu" style={{ left: `${contextMenu.x}px`, top: `${contextMenu.y}px` }} onContextMenu={(e) => {
                e.preventDefault();
                e.stopPropagation();
                setContextMenu(null);
                requestAnimationFrame(() => {
            ...
            {contextMenu.kind === "tentacle" && (<>
                <button type="button" className="canvas-context-menu-item" onClick={() => handleCreateAgent(contextMenu.tentacleId)}>
                  <span className="canvas-context-menu-icon">
                    <TerminalIcon size={14}/>
                  </span>
                  Create new agent
                ...
                <button type="button" className="canvas-context-menu-item" onClick={() => handleTentacleAction(contextMenu.tentacleId, "tentacle-reorganize-todos")}>
                  <span className="canvas-context-menu-icon">
                    <ListTodo size={14}/>
                  </span>
                  Update To-Do List
                </button>
                <button type="button" className="canvas-context-menu-item" onClick={() => handleTentacleAction(contextMenu.tentacleId, "tentacle-update-tentacle")}>
                  <span className="canvas-context-menu-icon">
                    <Hexagon size={14}/>
                  </span>
                  Update Tentacle
                </button>
                <button type="button" className="canvas-context-menu-item" onClick={() => handleSpawnSwarm(contextMenu.tentacleId, "worktree")}>
                  <span className="canvas-context-menu-icon">
                    <Layers size={14}/>
                  </span>
                  Spawn Swarm (Worktrees)
                </button>
                <button type="button" className="canvas-context-menu-item" onClick={() => handleSpawnSwarm(contextMenu.tentacleId, "shared")}>
                  <span className="canvas-context-menu-icon">
                    <Layers size={14}/>
                  </span>
                  Spawn Swarm (Normal)
            ...
            {contextMenu.kind === "octoboss" && (<>
                <button type="button" className="canvas-context-menu-item" onClick={() => handleOctobossAction("octoboss-reorganize-todos")}>
                  <span className="canvas-context-menu-icon">
                    <ListTodo size={14}/>
                  </span>
                  Reorganize To-Do's
                </button>
                <button type="button" className="canvas-context-menu-item" onClick={() => handleOctobossAction("octoboss-reorganize-tentacles")}>
                  <span className="canvas-context-menu-icon">
                    <Hexagon size={14}/>
                  </span>
                  Reorganize Tentacles
                </button>
                <button type="button" className="canvas-context-menu-item" onClick={() => handleOctobossAction("octoboss-clean-contexts")}>
                  <span className="canvas-context-menu-icon">
                    <Sparkles size={14}/>
                  </span>
            ...
            {contextMenu.kind === "active-session" && (<button type="button" className="canvas-context-menu-item canvas-context-menu-item--danger" onClick={() => {
      ...
      {pendingDeleteTerminal && onCancelDelete && onConfirmDelete && (<div className="canvas-delete-dialog">
          <DeleteTentacleDialog pendingDeleteTerminal={pendingDeleteTerminal} isDeletingTerminalId={isDeletingTerminalId ?? null} onCancel={onCancelDelete} onConfirmDelete={onConfirmDelete}/>
        </div>)}

      {isDeleteAllDialogOpen && (<div className="canvas-delete-dialog">
          <DeleteAllTerminalsDialog columns={columns} nodes={nodes} onCancel={() => setIsDeleteAllDialogOpen(false)} onDeleted={({ hadFailures }) => {
                if (!hadFailures) {
...
//# sourceMappingURL=CanvasPrimaryView.js.map