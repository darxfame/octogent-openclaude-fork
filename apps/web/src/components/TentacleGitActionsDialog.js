...
export const TentacleGitActionsDialog = ({ tentacleId, tentacleName, gitStatus, gitPullRequest, gitCommitMessage, isLoading, isMutating, errorMessage, onCommitMessageChange, onClose, onCommit, onCommitAndPush, onPush, onSync, onMergePullRequest, onCleanupWorktree, }) => {
    ...
    return (<section aria-label={`Git actions for ${tentacleId}`} className="git-actions-dialog" onKeyDown={(event) => {
            if (event.key === "Escape") {
                if (isCommitMenuOpen) {
      ...
      <header className="git-actions-header">
        ...
        <div className="git-actions-header-actions">
          ...
          <ActionButton aria-label="Close sidebar action panel" className="git-actions-close" disabled={isMutating} onClick={onClose} size="dense" variant="accent">
            Close
          </ActionButton>
        </div>
      </header>
      <div className="git-actions-body">
        ...
        {isLoading ? (<p className="git-actions-loading">Loading git status...</p>) : gitStatus ? (<dl className="git-actions-status">
            <div>
              <dt>Branch</dt>
          ...
          </dl>) : (<p className="git-actions-loading">No git status available.</p>)}
        ...
        <section className="git-actions-commit-panel" aria-label="Source control composer">
          ...
          <textarea aria-label={`Commit message for ${tentacleId}`} className="git-actions-message-input" id="git-actions-commit-input" onChange={(event) => {
          ...
          <div className="git-actions-commit-controls">
            <ActionButton aria-label="Commit changes" className="git-actions-commit-main" disabled={Boolean(commitDisabledReason)} onClick={onCommit} size="dense" variant="accent">
              {isMutating ? "Running..." : "Commit"}
            </ActionButton>
            <button aria-expanded={isCommitMenuOpen} aria-haspopup="menu" aria-label="Open commit options" className="git-actions-commit-toggle" disabled={Boolean(globalDisabledReason)} onClick={() => {
            setIsCommitMenuOpen((current) => !current);
        }} type="button">
              <ChevronDown size={14}/>
            </button>
          </div>
          {isCommitMenuOpen && (<div className="git-actions-commit-menu" role="menu">
              <button className="git-actions-commit-menu-item" disabled={Boolean(commitDisabledReason)} onClick={() => {
                setIsCommitMenuOpen(false);
                onCommit();
            }} role="menuitem" type="button">
                Commit
              </button>
              <button className="git-actions-commit-menu-item" disabled={Boolean(commitAndPushDisabledReason)} onClick={() => {
                setIsCommitMenuOpen(false);
                onCommitAndPush();
            }} role="menuitem" type="button">
                Commit & Push
              </button>
              <button className="git-actions-commit-menu-item" disabled={Boolean(pushDisabledReason)} onClick={() => {
                setIsCommitMenuOpen(false);
                onPush();
            }} role="menuitem" type="button">
                Push
              </button>
              <button className="git-actions-commit-menu-item" disabled={Boolean(syncDisabledReason)} onClick={() => {
                setIsCommitMenuOpen(false);
                onSync();
            }} role="menuitem" type="button">
                Sync with Base
          ...
          {syncDisabledReason ? (<p className="git-action-hint">{syncDisabledReason}</p>) : (<p className="git-action-hint">Sync is ready. Use the commit menu to run sync.</p>)}
        </section>
        ...
        <section className="git-actions-pr-section" aria-label="Pull request workflow">
          ...
          <div className="git-actions-pr-buttons">
            <ActionButton aria-label="Merge pull request" className="git-actions-merge-pr" disabled={Boolean(mergePullRequestDisabledReason)} onClick={onMergePullRequest} size="dense" variant="info">
              Merge pull request
            </ActionButton>
            <ActionButton aria-label="Open pull request in GitHub" className="git-actions-open-pr" disabled={!gitPullRequest?.url} onClick={() => {
            if (!gitPullRequest?.url) {
                return;
            }
            globalThis.open?.(gitPullRequest.url, "_blank", "noopener,noreferrer");
        }} size="dense" variant="accent">
              Open on GitHub
            </ActionButton>
          </div>
          {mergePullRequestDisabledReason && (<p className="git-action-reason">{mergePullRequestDisabledReason}</p>)}
          {!gitPullRequest?.url && (<p className="git-action-hint">No pull request URL detected for this branch yet.</p>)}
        </section>
        ...
        <div className="git-action-row git-action-row--cleanup">
          ...
          <ActionButton aria-label="Cleanup worktree" className="git-actions-cleanup" disabled={Boolean(cleanupDisabledReason)} onClick={onCleanupWorktree} size="dense" variant="danger">
            Cleanup worktree
          </ActionButton>
        </div>
...
//# sourceMappingURL=TentacleGitActionsDialog.js.map