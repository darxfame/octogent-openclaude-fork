...
const buildColorMap = (keys) => new Map(keys.map((k, i) => [
    k,
    SEGMENT_COLORS[i % SEGMENT_COLORS.length] ?? SEGMENT_COLORS[0] ?? "#ffffff",
]));
const buildBars = (days, keys, mode) => {
...
/* ── Tooltip (shared) ───────────────────────────────── */
const ChartTooltip = ({ bar, x, y, containerWidth, }) => {
    const isRightHalf = x > containerWidth / 2;
    return (<div className="usage-heatmap-tooltip" aria-live="polite" style={isRightHalf
      ...
      <dl className="usage-heatmap-tooltip-stats">
        ...
        {bar.segments.map((seg) => (<div key={seg.label}>
            <dt>
...
/* ── Trend curve ───────────────────────────────────── */
const buildTrendPath = (bars, maxTokens, chartHeight, yAxisWidth, topPad, barSlotWidth) => {
    if (bars.length < 2 || maxTokens === 0)
        return "";
    const LIFT = 8;
...
const BarChartView = ({ bars, maxTokens, containerWidth, containerHeight, hoveredBar, setHoveredBar, }) => {
    ...
    const trendPath = useMemo(() => buildTrendPath(bars, maxTokens, chartHeight, Y_AXIS_WIDTH, TOP_PAD, barSlotWidth), [bars, maxTokens, chartHeight, barSlotWidth]);
    return (<svg className="usage-chart-svg" viewBox={`0 0 ${containerWidth} ${svgHeight}`} role="img" aria-label="Token usage bar chart">
      {yTicks.map((tick) => {
            const y = TOP_PAD + chartHeight - (maxTokens > 0 ? (tick.value / maxTokens) * chartHeight : 0);
            return (<g key={tick.value}>
            <line x1={Y_AXIS_WIDTH} y1={y} x2={containerWidth} y2={y} className="usage-chart-grid-line"/>
      ...
      {bars.map((bar, i) => {
            ...
            return (<g key={bar.date} onMouseEnter={() => setHoveredBar(bar)} onMouseLeave={() => setHoveredBar(null)} className="usage-chart-bar-group">
            <rect x={x} y={TOP_PAD} width={barWidth} height={chartHeight} fill="transparent" className="usage-chart-bar-hit"/>
            {bar.segments.map((seg) => {
                    const segHeight = maxTokens > 0 ? (seg.tokens / maxTokens) * chartHeight : 0;
                    yOffset -= segHeight;
                    return (<rect key={seg.label} x={x} y={yOffset} width={barWidth} height={Math.max(0.5, segHeight)} fill={seg.color} rx={1}/>);
                })}
          </g>);
        })}
      ...
      {bars.map((bar, i) => {
            ...
            return (<text key={`label-${bar.date}`} x={x} y={TOP_PAD + chartHeight + X_LABEL_HEIGHT - 2} className="usage-chart-x-label">
            {formatDateLabel(bar.date)}
          </text>);
        })}

      {trendPath && (<path d={trendPath} className="usage-chart-trend-line" fill="none" stroke="rgba(215, 166, 34, 0.55)" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"/>)}
    </svg>);
};
/* ── Heatmap view ───────────────────────────────────── */
const CELL_GAP = 3;
...
const HeatmapView = ({ bars, containerWidth, containerHeight, hoveredBar, setHoveredBar, }) => {
    ...
    const cellSize = Math.max(8, Math.min(Math.floor((availableHeight - 6 * CELL_GAP) / 7), Math.floor((availableWidth - (WEEKS_TO_SHOW - 1) * CELL_GAP) / WEEKS_TO_SHOW)));
    ...
    return (<svg className="usage-chart-svg usage-chart-svg--heatmap" viewBox={`0 0 ${svgWidth} ${svgHeight}`} width={svgWidth} height={svgHeight} role="img" aria-label="Token usage heatmap">
      {monthLabels.map(({ label, week }) => (<text key={`month-${week}`} x={dayLabelWidth + week * (cellSize + CELL_GAP)} y={monthLabelHeight - 4} className="usage-heatmap-month-label">
          {label}
        </text>))}

      {DAY_LABELS.map((label, dayIndex) => label ? (<text key={label} x={dayLabelWidth - 6} y={monthLabelHeight + dayIndex * (cellSize + CELL_GAP) + cellSize - 2} className="usage-heatmap-day-label">
            {label}
          </text>) : null)}

      {cells.map((cell) => (<rect key={cell.date} x={dayLabelWidth + cell.week * (cellSize + CELL_GAP)} y={monthLabelHeight + cell.dayOfWeek * (cellSize + CELL_GAP)} width={cellSize} height={cellSize} rx={CELL_RADIUS} fill={INTENSITY_COLORS[cell.intensity] ?? INTENSITY_COLORS[0] ?? "transparent"} className="usage-heatmap-cell" onMouseEnter={() => {
                if (cell.bar)
                    setHoveredBar(cell.bar);
...
/* ── Main component ─────────────────────────────────── */
export const UsageBarChart = ({ data, isLoading, onRefresh }) => {
    const [segmentMode, setSegmentMode] = useState("project");
    ...
    const bars = useMemo(() => buildBars(days, segmentKeys, segmentMode), [days, segmentKeys, segmentMode]);
    ...
    const stats = useMemo(() => {
        ...
        if (!firstDay)
            return null;
        const peakDay = days.reduce((best, d) => (d.totalTokens > best.totalTokens ? d : best), firstDay);
    ...
    return (<section className="usage-heatmap" aria-label="Claude token usage chart">
      <header className="usage-heatmap-header">
        ...
        <div className="usage-heatmap-header-actions">
          <ActionButton aria-label="Refresh usage chart data" className="usage-heatmap-refresh" disabled={isLoading} onClick={onRefresh} size="dense" variant="accent">
            {isLoading ? "Scanning..." : "Refresh"}
          </ActionButton>
        </div>
      </header>

      <div className="usage-chart-split" ref={splitRef} onMouseMove={(e) => {
        ...
        <div className="usage-chart-bar-segment-toggle">
          <button type="button" className={`usage-chart-bar-segment-btn${segmentMode === "project" ? " is-active" : ""}`} onClick={() => setSegmentMode("project")}>
            Project
          </button>
          <button type="button" className={`usage-chart-bar-segment-btn${segmentMode === "model" ? " is-active" : ""}`} onClick={() => setSegmentMode("model")}>
            Model
          </button>
        </div>
        <div className="usage-chart-left-stack">
          <div className="usage-chart-panel" ref={barPanel.ref}>
            <BarChartView bars={bars} maxTokens={maxTokens} containerWidth={barPanel.width} containerHeight={barPanel.height} hoveredBar={hoveredBar} setHoveredBar={setHoveredBar}/>
          </div>
          {segmentKeys.length > 1 && (<div className="usage-chart-legend">
              {segmentKeys.map((key) => (<span key={key} className="usage-chart-legend-item">
                  <span className="usage-chart-legend-dot" style={{ backgroundColor: colorMap.get(key) }}/>
                  {key}
                </span>))}
            </div>)}
        </div>
        <div className="usage-chart-right-stack">
          <div className="usage-chart-panel" ref={heatmapPanel.ref}>
            <HeatmapView bars={heatmapBars} containerWidth={heatmapPanel.width} containerHeight={heatmapPanel.height} hoveredBar={hoveredBar} setHoveredBar={setHoveredBar}/>
          </div>
          {stats && (<dl className="usage-chart-stats">
        ...
        {hoveredBar && hoveredBar.totalTokens > 0 && (<ChartTooltip bar={hoveredBar} x={mousePos.x} y={mousePos.y} containerWidth={splitRef.current?.clientWidth ?? 800}/>)}
      </div>
    </section>);
};
