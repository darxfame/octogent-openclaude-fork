...
export const CodeIntelTreemap = ({ root }) => {
    ...
    return (<div className="code-intel-treemap" ref={containerRef} onMouseMove={handleMouseMove}>
      <svg className="code-intel-treemap-svg" viewBox={`0 0 ${size.width} ${size.height}`} width={size.width} height={size.height} role="img" aria-label="File edit frequency treemap">
        {rects.map((r) => {
            const gapX = GAP;
            const gapY = GAP;
            const rx = r.x + gapX / 2;
            ...
            return (<g key={r.path} onMouseEnter={() => setHoveredRect(r)} onMouseLeave={() => setHoveredRect(null)}>
              <rect x={rx} y={ry} width={rw} height={rh} rx={2} fill={heatColor(r.value, maxValue)} className="code-intel-treemap-cell" style={{
                    filter: isHovered ? "brightness(1.35)" : undefined,
                    stroke: isHovered ? "#d4a017" : "#0b0d10",
                    strokeWidth: isHovered ? 1.5 : 0.5,
                }}/>
              {showLabel && (<text x={rx + 4} y={ry + 13} className="code-intel-treemap-label" clipPath={"inset(0 0 0 0)"}>
      ...
      {hoveredRect && (<div className="code-intel-tooltip" style={{
...
//# sourceMappingURL=CodeIntelTreemap.js.map