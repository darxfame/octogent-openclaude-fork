...
function buildHead(face, topRows = HEAD_TOP, bodyRows = HEAD_BODY) {
...
const SPRITE_H = HEAD_TOP.length +
    FACE_NORMAL.length +
    HEAD_BODY.length +
    TENTACLE_TOP.length +
    TAIL_NEUTRAL.length;
...
// ─── Drawing ─────────────────────────────────────────────────────────────────
function drawSprite(ctx, accentColor, frame, head, scale, topPad) {
...
function drawZZZ(ctx, scale, zzzPhase) {
    if (zzzPhase >= 3)
        return; // phases 3-4 are the blank pause
...
function drawAccessory(ctx, accessory, scale, yOff, hColor) {
    if (accessory === "none")
        return;
...
function animationFrameMs(animation) {
    if (animation === "jog" || animation === "swim-up")
        return JOG_FRAME_MS;
    if (animation === "walk")
        return WALK_FRAME_MS;
    if (animation === "float")
        return FLOAT_FRAME_MS;
    return SWAY_FRAME_MS;
}
const IDLE_FRAME = { bottom: [...TENTACLE_TOP, ...TAIL_NEUTRAL] };
export const OctopusGlyph = ({ animation = "sway", expression = "normal", accessory = "none", hairColor = HAIR_COLOR, scale = DEFAULT_SCALE, className, color, testId, }) => {
    const canvasRef = useRef(null);
    const frameRef = useRef(0);
    const zzzPhaseRef = useRef(0);
    // Extra canvas rows above the sprite for overlays (ZZZ, accessories).
    const topPad = Math.max(expression === "sleepy" ? ZZZ_PAD : 0, accessory !== "none" ? ACCESSORY_PAD : 0);
    useEffect(() => {
        ...
        const drawFrame = (frame, zzzPhase) => {
            drawSprite(ctx, accentColor, frame, head, scale, topPad);
            if (expression === "sleepy")
                drawZZZ(ctx, scale, zzzPhase);
            const yOff = (frame.yOffset ?? 0) + topPad;
    ...
    return (<canvas ref={canvasRef} className={className} width={SPRITE_W * scale} height={(topPad + SPRITE_H + BOUNCE_PAD) * scale} data-testid={testId}/>);
};
//# sourceMappingURL=EmptyOctopus.js.map