...
const normalizePostSnippet = (value) => value.replace(/\s+/g, " ").trim();
...
const buildTelemetryItems = (monitorFeed) => {
...
export const TelemetryTape = ({ monitorFeed }) => {
    ...
    return (<section className="console-telemetry-tape" aria-label="Telemetry ticker tape">
      <div className="console-telemetry-track" style={{ animationDuration: `${scrollDurationSeconds}s` }}>
        {[...telemetryItems, ...telemetryItems].map((item, index) => item.permalink ? (<a className="console-telemetry-item" href={item.permalink} key={`${item.key}-${index}`} rel="noreferrer" target="_blank" title={item.fullText}>
            ...
            </a>) : (<span className="console-telemetry-item" key={`${item.key}-${index}`} title={item.fullText}>
              <span aria-hidden="true" className="console-telemetry-source">
                {item.sourceIcon}
              </span>
              <strong>{item.authorLabel}</strong>
...
//# sourceMappingURL=TelemetryTape.js.map