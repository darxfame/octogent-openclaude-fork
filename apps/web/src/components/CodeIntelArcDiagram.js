...
const MAX_ARCS = 40;
const ACCENT = "#d4a017";
export const CodeIntelArcDiagram = ({ data }) => {
    ...
    const fileY = (index) => PADDING_Y + index * dynamicRowHeight + dynamicRowHeight / 2;
    ...
    return (<div className="code-intel-arc-diagram" ref={containerRef} onMouseMove={handleMouseMove}>
      <svg className="code-intel-arc-svg" width={size.width} height={Math.max(svgHeight, size.height)} viewBox={`0 0 ${size.width} ${Math.max(svgHeight, size.height)}`} role="img" aria-label="File coupling arc diagram">
        ...
        {pairs.map((pair) => {
            ...
            const isHovered = hoveredPair === key || hoveredFile === pair.fileA || hoveredFile === pair.fileB;
            const d = `M ${dotX} ${y1} C ${curveX} ${y1}, ${curveX} ${y2}, ${dotX} ${y2}`;
            return (<g key={key}>
              <path d={d} fill="none" stroke="transparent" strokeWidth={12} onMouseEnter={() => setHoveredPair(key)} onMouseLeave={() => setHoveredPair(null)} style={{ cursor: "crosshair" }}/>
              <path d={d} fill="none" stroke={heatColor(pair.coSessions, maxCoSessions)} strokeWidth={isHovered ? 1.5 : 0.75} strokeOpacity={isHovered ? 1 : 0.4} className="code-intel-arc-path" pointerEvents="none"/>
            </g>);
        })}
        ...
        {files.map((f, i) => {
            const y = fileY(i);
            const isHighlighted = hoveredFile === f.file ||
                (hoveredPair !== null &&
                    pairs.some((p) => pairKey(p.fileA, p.fileB) === hoveredPair &&
                        (p.fileA === f.file || p.fileB === f.file)));
            const shortName = f.file.split("/").pop() ?? f.file;
            return (<g key={f.file} onMouseEnter={() => setHoveredFile(f.file)} onMouseLeave={() => setHoveredFile(null)} className="code-intel-arc-file-group">
              <circle cx={dotX} cy={y} r={3.5} fill={ACCENT} fillOpacity={isHighlighted ? 1 : 0.7} stroke={isHighlighted ? "#fff" : "none"} strokeWidth={isHighlighted ? 1.5 : 0}/>
              <text x={dotX - 8} y={y + 3.5} textAnchor="end" className={`code-intel-arc-label${isHighlighted ? " code-intel-arc-label--active" : ""}`}>
      ...
      {/* Edge hover tooltip */}
      {hoveredPairData && !hoveredFile && (<div className="code-intel-tooltip" style={{
      ...
      {hoveredFile && (<div className="code-intel-tooltip code-intel-tooltip--node" style={{
                left: dotX + 12,
                top: Math.max(fileY(fileIndexMap.get(hoveredFile) ?? 0) - 20, 4),
            }}>
          <div className="code-intel-tooltip-path">{hoveredFile}</div>
          {fileCouplingMap.has(hoveredFile) ? (<div className="code-intel-tooltip-coupling-list">
              {fileCouplingMap.get(hoveredFile)?.map((c) => (<div key={c.partner} className="code-intel-tooltip-coupling-row">
            ...
            </div>) : (<div className="code-intel-tooltip-value">No coupling detected</div>)}
        </div>)}
    </div>);
};
const pairKey = (a, b) => (a < b ? `${a}\0${b}` : `${b}\0${a}`);
const shortFileName = (path) => path.split("/").pop() ?? path;
...
//# sourceMappingURL=CodeIntelArcDiagram.js.map