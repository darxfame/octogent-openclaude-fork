...
import { buildDeckSkillsUrl, buildDeckTentacleSkillsUrl, buildDeckTentacleUrl, buildDeckTentaclesUrl, buildDeckTodoToggleUrl, buildDeckVaultFileUrl, buildTerminalsUrl, } from "../runtime/runtimeEndpoints";
...
import { deriveOctopusVisuals } from "./deck/octopusVisuals";
...
export const DeckPrimaryView = ({ onSidebarContent, workspaceSetup, isWorkspaceSetupLoading, workspaceSetupError, onRefreshWorkspaceSetup, onRunWorkspaceSetupStep, suppressWorkspaceSetupCard = false, }) => {
    ...
    const [savingTentacleSkillsId, setSavingTentacleSkillsId] = useState(null);
    ...
    const handleVaultFileClick = useCallback((tentacleId, fileName) => {
    ...
    const handleLaunchAgent = useCallback(async () => {
        ...
        try {
            ...
            if (!response.ok)
                return;
            const data = await response.json();
            const agentId = (data.terminalId ?? data.tentacleId);
    ...
    const handleCreateTentacle = useCallback(async (name, description, color, octopus, suggestedSkills) => {
        setIsCreating(true);
        setCreateError(null);
        try {
            const response = await fetch(buildDeckTentaclesUrl(), {
    ...
    const handleTentacleSkillsSave = useCallback(async (tentacleId, suggestedSkills) => {
    ...
    const [deletingTentacleId, setDeletingTentacleId] = useState(null);
    const handleDeleteTentacle = useCallback(async (tentacleId) => {
    ...
    const handleTodoToggle = useCallback(async (tentacleId, itemIndex, done) => {
        try {
    ...
    const focusedTentacle = focus?.type === "vault" || focus?.type === "vault-browser"
        ? tentacles.find((t) => t.tentacleId === focus.tentacleId)
        : null;
    const mode = focus ? "detail" : "grid";
    const shouldShowWorkspaceSetup = !suppressWorkspaceSetupCard && tentacles.length === 0 && workspaceSetup?.shouldShowSetupCard;
    // Push sidebar content to the shared sidebar
    const sidebarContent = useMemo(() => tentacles.length > 0 || focus?.type === "terminal" || shouldShowWorkspaceSetup ? (<div className="deck-sidebar-content">
          <div className="deck-sidebar-content-top">
            {shouldShowWorkspaceSetup ? (<WorkspaceSetupCard compact workspaceSetup={workspaceSetup} isLoading={isWorkspaceSetupLoading} error={workspaceSetupError} onRunStep={handleRunSetupStep} onLaunchClaudeCode={handleLaunchAgent} isLaunchingAgent={isLaunchingAgent} isRunningStepId={runningSetupStepId}/>) : (<ActionCards compact selectedAgent={selectedAgent} setSelectedAgent={setSelectedAgent} agentMenuOpen={agentMenuOpen} setAgentMenuOpen={setAgentMenuOpen} agentMenuRef={agentMenuRef} onAddManually={() => {
                setEmptyViewMode("adding");
                setCreateError(null);
            }} onLaunchAgent={handleLaunchAgent} isLaunchingAgent={isLaunchingAgent}/>)}
          </div>
          {tentacles.length > 0 && (<div className="deck-sidebar-content-bottom">
              <DeckBottomActions onClearAll={async () => {
                for (const t of tentacles) {
    ...
    if (tentacles.length === 0 && focus?.type !== "terminal") {
        return (<section className="deck-view" data-mode="grid" data-empty-mode={emptyViewMode} aria-label="Deck">
        <div className="deck-empty-state">
          <div className="deck-empty-left">
            <div className="deck-empty-octopus">
              <OctopusGlyph color="#d4a017" animation="walk" expression="happy" accessory="none" scale={20}/>
            </div>
            {shouldShowWorkspaceSetup ? (<WorkspaceSetupCard workspaceSetup={workspaceSetup} isLoading={isWorkspaceSetupLoading} error={workspaceSetupError} onRunStep={handleRunSetupStep} onLaunchClaudeCode={handleLaunchAgent} isLaunchingAgent={isLaunchingAgent} isRunningStepId={runningSetupStepId}/>) : (<ActionCards selectedAgent={selectedAgent} setSelectedAgent={setSelectedAgent} agentMenuOpen={agentMenuOpen} setAgentMenuOpen={setAgentMenuOpen} agentMenuRef={agentMenuRef} onAddManually={() => {
                    setEmptyViewMode("adding");
                    setCreateError(null);
                }} onLaunchAgent={handleLaunchAgent} isLaunchingAgent={isLaunchingAgent}/>)}
          </div>
          {emptyViewMode === "adding" && (<div className="deck-empty-right">
              <AddTentacleForm onSubmit={handleCreateTentacle} onCancel={() => setEmptyViewMode("idle")} isSubmitting={isCreating} error={createError} availableSkills={availableSkills}/>
            </div>)}
        </div>
      </section>);
    }
    // ─── Populated state ────────────────────────────────────────────────────────
    return (<section className="deck-view" data-mode={mode} data-has-pods={tentacles.length > 0} aria-label="Deck">
      <div className="deck-pods-container">
        {tentacles.map((t) => {
            const isThis = (focus?.type === "vault" || focus?.type === "vault-browser") &&
                focus.tentacleId === t.tentacleId;
            return (<div key={t.tentacleId} className="deck-pod-slot" data-pod-role={isThis ? "focused" : focus ? "other" : "idle"}>
              <TentaclePod tentacle={t} visuals={visualsMap.get(t.tentacleId)} isFocused={isThis} activeFileName={focus?.type === "vault" && isThis ? focus.fileName : undefined} onVaultFileClick={(fileName) => setFocus({ type: "vault", tentacleId: t.tentacleId, fileName })} onVaultBrowse={() => setFocus({ type: "vault-browser", tentacleId: t.tentacleId })} onClose={handleClose} onDelete={() => handleDeleteTentacle(t.tentacleId)} isDeleting={deletingTentacleId === t.tentacleId} onTodoToggle={handleTodoToggle} availableSkills={availableSkills} isSavingSkills={savingTentacleSkillsId === t.tentacleId} onSaveSuggestedSkills={handleTentacleSkillsSave}/>
            </div>);
        })}
      </div>
      ...
      <div className="deck-detail-main">
            ...
            <div className="deck-detail-main-content deck-vault-browser">
              <pre className="deck-vault-tree">
                ...
                return files.map((file, i) => {
                    ...
                    const prefix = isLast ? "└── " : "├── ";
                    return (<span key={file} className="deck-vault-tree-row">
                        <span className="deck-vault-tree-branch">{prefix}</span>
                        <button type="button" className="deck-vault-tree-file" onClick={() => setFocus({
                            type: "vault",
        ...
        {focus?.type === "vault" && focusedTentacle && (<>
            <header className="deck-detail-main-header">
              <button type="button" className="deck-add-form-back" onClick={() => setFocus({ type: "vault-browser", tentacleId: focus.tentacleId })}>
                ← Back
              </button>
              <span className="deck-detail-main-path">
            ...
            <div className="deck-detail-main-content" key={`${focus.tentacleId}/${focus.fileName}`}>
              {loadingVault ? (<span className="deck-detail-loading">Loading…</span>) : vaultContent !== null ? (<MarkdownContent content={vaultContent} className="deck-detail-markdown"/>) : (<span className="deck-detail-loading">File not found.</span>)}
            </div>
          </>)}
        {focus?.type === "terminal" && (<div className="deck-detail-terminal" key={focus.agentId}>
            <header className="deck-detail-main-header">
              <button type="button" className="deck-add-form-back" onClick={handleClose}>
                ← Back
              </button>
...
//# sourceMappingURL=DeckPrimaryView.js.map