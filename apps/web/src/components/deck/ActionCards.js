...
export const ActionCards = ({ compact, selectedAgent, setSelectedAgent, agentMenuOpen, setAgentMenuOpen, agentMenuRef, onAddManually, onLaunchAgent, isLaunchingAgent, }) => (<div className={`deck-empty-actions${compact ? " deck-empty-actions--compact" : ""}`}>
    <button type="button" className="deck-empty-card" onClick={onAddManually}>
      <div className="deck-empty-card-icon">
        <OctopusGlyph color="#d4a017" animation="idle" expression="normal" accessory="none" scale={compact ? 3 : 4}/>
      </div>
      ...
      <div className="deck-empty-card-text">
        ...
        <div className="deck-empty-agent-select-row">
          <div className="deck-empty-agent-picker" ref={agentMenuRef}>
            <button type="button" className="deck-empty-agent-trigger" aria-expanded={agentMenuOpen} aria-haspopup="menu" onClick={() => setAgentMenuOpen((p) => !p)}>
            ...
            {agentMenuOpen && (<div className="deck-empty-agent-menu" role="menu">
                {AGENT_PROVIDER_OPTIONS.map((opt) => (<button key={opt.value} type="button" className="deck-empty-agent-menu-item" role="menuitem" data-active={opt.value === selectedAgent ? "true" : "false"} onClick={() => {
                setSelectedAgent(opt.value);
                setAgentMenuOpen(false);
            }}>
                    {opt.label}
                    {opt.value === selectedAgent && (<span className="deck-empty-agent-menu-check">
                        <Check size={12}/>
                      </span>)}
                  </button>))}
              </div>)}
          </div>
          <button type="button" className="deck-empty-agent-launch" disabled={isLaunchingAgent} onClick={onLaunchAgent}>
...
//# sourceMappingURL=ActionCards.js.map