...
export const GitHubPrimaryView = ({ githubRepoLabel, githubStatusPill, isRefreshingGitHubSummary, onRefresh, githubStarCountLabel, githubOpenIssuesLabel, githubOpenPrsLabel, githubRecentCommits, githubCommitCount30d, githubOverviewHoverLabel, githubOverviewGraphPolylinePoints, githubOverviewGraphSeries, hoveredGitHubOverviewPointIndex, onHoveredGitHubOverviewPointIndexChange, }) => {
    ...
    const tooltipLabel = hoveredGitHubOverviewPoint
        ...
        : null;
    return (<section className="github-view" aria-label="GitHub primary view">
      <section className="github-overview" aria-label="GitHub overview">
        <header className="github-overview-header">
          ...
          <div className="github-overview-header-actions">
            ...
            <ActionButton aria-label="Refresh GitHub overview data" className="github-overview-refresh" disabled={isRefreshingGitHubSummary} onClick={onRefresh} size="dense" variant="accent">
        ...
        <div className="github-overview-content">
          <section className="github-overview-main">
            ...
            setHoverCursorPosition({
                ...
                y: Math.max(0, Math.min(rect.height, pointerY)),
        ...
        }} viewBox={`${-GITHUB_OVERVIEW_GRAPH_VIEWBOX_INSET} ${-GITHUB_OVERVIEW_GRAPH_VIEWBOX_INSET} ${GITHUB_OVERVIEW_GRAPH_WIDTH + GITHUB_OVERVIEW_GRAPH_VIEWBOX_INSET * 2} ${GITHUB_OVERVIEW_GRAPH_HEIGHT + GITHUB_OVERVIEW_GRAPH_VIEWBOX_INSET * 2}`} preserveAspectRatio="none" role="presentation">
                  <defs>
                  ...
                  {yTicks.map((tick, index) => (<g key={`${tick.count}-${index}`}>
                      <line x1={0} y1={tick.y} x2={GITHUB_OVERVIEW_GRAPH_WIDTH} y2={tick.y} className="github-overview-graph-grid"/>
                      <text x={4} y={tick.y - 4} className="github-overview-graph-y-label">
                        {tick.count}
                      </text>
                    </g>))}

                  {areaPolygonPoints && (<polygon points={areaPolygonPoints} fill="url(#commitAreaGrad)"/>)}
            ...
            if (!label)
                return null;
            return (<text key={`xl-${point.date}`} x={point.x} y={GITHUB_OVERVIEW_GRAPH_HEIGHT + GITHUB_OVERVIEW_GRAPH_VIEWBOX_INSET} className="github-overview-graph-x-label">
                          {label}
                        </text>);
        })}

                  {githubOverviewGraphSeries.map((point, index) => (<circle aria-label={formatGitHubCommitHoverLabel(point)} className={`github-overview-graph-point${hoveredGitHubOverviewPointIndex === index ? " is-active" : ""}`} cx={point.x} cy={point.y} key={`${point.date}-${index}`} onFocus={() => {
                ...
                {hoverCursorPosition && tooltipLabel && (<div className="github-overview-graph-tooltip" style={{
          ...
          <aside className="github-overview-side" aria-label="GitHub recent activity">
            <dl className="github-overview-stats" aria-label="Repository stats">
              <div aria-label={`Stars ${githubStarCountLabel}`} className="github-overview-stat" data-metric="st" data-label="Stars" title="Stars">
                <dt>
              ...
              <div aria-label={`Open issues ${githubOpenIssuesLabel}`} className="github-overview-stat" data-metric="is" data-label="Open issues" title="Open issues">
                <dt>
                  <span aria-hidden="true" className="github-overview-stat-icon">
                    <svg aria-hidden="true" focusable="false" viewBox="0 0 16 16">
              ...
              <div aria-label={`Open PRs ${githubOpenPrsLabel}`} className="github-overview-stat" data-metric="pr" data-label="Open PRs" title="Open PRs">
                <dt>
                  <span aria-hidden="true" className="github-overview-stat-icon">
                    <svg aria-hidden="true" focusable="false" viewBox="0 0 16 16">
              ...
              <div aria-label={`Commits in 30 days ${githubCommitCount30d}`} className="github-overview-stat" data-metric="30d" data-label="Commits (30d)" title="Commits (30d)">
                <dt>
                  <span aria-hidden="true" className="github-overview-stat-icon">
                    <svg aria-hidden="true" focusable="false" viewBox="0 0 16 16">
            ...
            <section className="github-overview-recent" aria-label="Recent commits" ref={recentSectionRef}>
              <header className="github-overview-recent-header">
                <h3>Recent commits</h3>
                <span>{`Showing last ${GITHUB_RECENT_COMMITS_LIMIT}`}</span>
              </header>
              {githubRecentCommits.length > 0 ? (<ol className="github-overview-recent-list">
                  {githubRecentCommits.map((commit) => (<li key={commit.hash}>
                      <button type="button" className={`github-overview-recent-item${pinnedCommitHash === commit.hash ? " is-selected" : ""}`} onMouseEnter={(event) => {
                    if (pinnedCommitHash) {
                        return;
                    }
                ...
                </ol>) : (<p className="github-overview-recent-empty">Recent commit data is unavailable.</p>)}
              <div ref={tooltipRef} className={`github-overview-recent-tooltip${activeCommit ? " is-visible" : ""}`} style={{
            top: commitTooltipY !== null ? `${commitTooltipY}px` : undefined,
                ...
                {activeCommit && (<>
                    <p className="github-overview-recent-tooltip-hash">
                      ...
                      <button className="github-overview-recent-tooltip-copy" type="button" title="Copy full hash" onClick={() => {
                ...
                : activeCommit.subject}
                    ...
                    {activeCommit.filesChanged > 0 && (<p className="github-overview-recent-tooltip-diff">
                        <span>
...
//# sourceMappingURL=GitHubPrimaryView.js.map