...
import { AgentStateBadge, isAgentRuntimeState } from "./AgentStateBadge";
...
const PromptInjectIcon = () => (<svg aria-hidden="true" className="terminal-inject-icon" viewBox="0 0 16 16" width="14" height="14">
    <path d="M2 3h12v1H2V3Zm0 3h8v1H2V6Zm0 3h6v1H2V9Zm9 0l3 2.5L11 14v-5Z" fill="currentColor"/>
  </svg>);
export const Terminal = ({ terminalId, terminalLabel, layoutVersion, isSelected, initialPrompt, hidePromptPicker, onSelectTerminal, onAgentRuntimeStateChange, onTerminalRenamed, onTerminalActivity, }) => {
    const socketRef = useRef(null);
    const containerRef = useRef(null);
    const [connectionState, setConnectionState] = useState("connecting");
    const [agentState, setAgentRuntimeState] = useState("idle");
    const [isPromptBannerDismissed, setIsPromptBannerDismissed] = useState(false);
    const [isPromptPickerOpen, setIsPromptPickerOpen] = useState(false);
    const promptPickerBtnRef = useRef(null);
    const terminalRef = useRef(null);
    const fitAddonRef = useRef(null);
    const requestResizeSyncRef = useRef(() => { });
    ...
    const handlePromptPickerSelect = useCallback((content) => {
            ...
            nextSocket.addEventListener("message", (event) => {
                ...
                try {
                    ...
                    if (payload.type === "history" && typeof payload.data === "string") {
                        if (activeTerminal) {
                            const viewport = containerRef.current?.querySelector(".xterm-viewport") ?? null;
        ...
        void (async () => {
            ...
            try {
                ...
                const terminalFontSize = Number.isFinite(rootFontSize)
                    ...
                    : 13;
                const terminalBackground = window
                ...
                const viewportWheelTarget = wheelListenerTarget.querySelector(".xterm-viewport") ?? wheelListenerTarget;
                const onPointerDown = () => {
                    terminal.focus();
                };
                const onWheel = (event) => {
    ...
    return (<div className={`terminal-pane${isSelected ? " terminal-pane--selected" : ""}`} data-selected={isSelected ? "true" : "false"} onFocusCapture={() => {
      ...
      <div className="terminal-header" data-connection-state={connectionState}>
        ...
        {initialPrompt && !isPromptBannerDismissed && (<div className="terminal-prompt-banner">
            <button type="button" className="terminal-prompt-banner-inject" onClick={() => {
                const ws = socketRef.current;
                if (ws && ws.readyState === 1) {
            ...
            <button type="button" className="terminal-prompt-banner-close" aria-label="Dismiss prompt" onClick={() => {
                setIsPromptBannerDismissed(true);
            }}>
              <X size={14}/>
            </button>
        ...
        <div className="terminal-header-actions">
          {!hidePromptPicker && (<div className="terminal-prompt-picker-anchor">
              <button ref={promptPickerBtnRef} type="button" className="terminal-prompt-picker-btn" title="Insert prompt" aria-label="Insert prompt" onClick={() => {
                setIsPromptPickerOpen((prev) => !prev);
            }}>
                <FileText size={13}/>
                <span className="terminal-prompt-picker-label">Prompts</span>
              </button>
              <TerminalPromptPicker isOpen={isPromptPickerOpen} anchorRef={promptPickerBtnRef} onClose={() => {
      ...
      <div ref={containerRef} className="terminal-mount" data-testid={`terminal-${terminalId}`} aria-label={`Terminal ${terminalId}`}/>
    </div>);
};
