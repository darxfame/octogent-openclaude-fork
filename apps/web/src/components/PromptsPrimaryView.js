...
export const PromptsPrimaryView = ({ enabled, onSidebarContent }) => {
    ...
    const sidebarContent = (<SidebarPromptsList prompts={prompts} selectedPromptName={selectedPromptName} isLoadingPrompts={isLoadingPrompts} onSelectPrompt={selectPromptLibraryItem} onRefresh={() => {
            void refreshPrompts();
        }} onNewPrompt={() => {
    ...
    return (<section className="prompts-view" aria-label="Prompts primary view">
      {/* Terminal — kept mounted when active, hidden via CSS when viewing a prompt */}
      {newPromptMode && (<div className="prompts-terminal" key={newPromptMode.terminalId} style={showTerminal ? undefined : { display: "none" }}>
      ...
      {showPromptDetail && (<>
          ...
          {isLoadingDetail ? (<p className="prompts-empty">Loading prompt...</p>) : selectedPrompt ? (<div className="prompts-detail">
              <header className="prompts-detail-header">
                ...
                {selectedPrompt.source === "user" && (<div className="prompts-detail-header-actions">
                    {isEditing ? (<>
              ...
              {isEditing ? (<textarea className="prompts-edit-area" value={editDraft} onChange={(e) => {
                        onSetEditDraft(e.target.value);
                    }} spellCheck={false}/>) : (<div className="prompts-content">
                  <MarkdownContent content={selectedPrompt.content}/>
                </div>)}
            </div>) : (<div className="prompts-empty-state">
...
//# sourceMappingURL=PromptsPrimaryView.js.map