...
export const DeleteAllTerminalsDialog = ({ columns, nodes, onCancel, onDeleted, }) => {
    ...
    const handleConfirm = useCallback(async () => {
        ...
        for (const terminal of activeTargets) {
            try {
                ...
                if (!response.ok) {
                    failures.push(`${terminal.tentacleName || terminal.label || terminal.terminalId}: ${await readDeleteFailureMessage(response, `Delete failed (${response.status})`)}`);
                }
            }
            catch (error) {
                failures.push(`${terminal.tentacleName || terminal.label || terminal.terminalId}: ${error instanceof Error ? error.message : "Delete failed."}`);
            }
            done += 1;
    ...
    return (<section aria-label="Delete all terminals" className="delete-confirm-dialog" onKeyDown={(event) => {
      ...
      <header className="delete-confirm-header">
        ...
        <div className="delete-confirm-header-actions">
          ...
          <ActionButton aria-label="Close confirmation" className="delete-confirm-close" disabled={isDeleting} onClick={onCancel} size="dense" variant="accent">
            Close
          </ActionButton>
        </div>
      </header>
      <div className="delete-confirm-body">
        ...
        {failureMessages.length > 0 && (<p className="delete-confirm-message" role="alert">
        ...
        <div className="delete-all-mode-row">
          ...
          <button type="button" className="delete-all-toggle-switch" role="switch" aria-checked={!inactiveOnly} aria-label="Toggle between inactive only and all terminals" disabled={isDeleting} onClick={() => setInactiveOnly((prev) => !prev)}>
        ...
        {progress && (<div className="delete-all-progress">
            Deleting... {progress.done}/{progress.total}
          </div>)}
      </div>
      <div className="delete-confirm-actions">
        <ActionButton aria-label="Cancel delete all" className="delete-confirm-cancel" disabled={isDeleting} onClick={onCancel} size="dense" variant="accent">
          Cancel
        </ActionButton>
        <ActionButton aria-label="Confirm delete all terminals" className="delete-confirm-submit" disabled={isDeleting || totalTargetCount === 0} onClick={() => void handleConfirm()} size="dense" variant="danger">
          {isDeleting
...
//# sourceMappingURL=DeleteAllTerminalsDialog.js.map