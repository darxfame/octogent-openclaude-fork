...
function deriveOctopusVisuals(node) {
...
export const OctopusNode = ({ node, connectedNodes, isSelected, selectedNodeId, selectedNodeColor, onPointerDown, onClick, }) => {
    const showFocus = isSelected;
    const isOctoboss = node.type === "octoboss";
    const lines = useMemo(() => splitLabel(node.label), [node.label]);
    const visuals = useMemo(() => isOctoboss
        ? { animation: "sway", expression: "normal", accessory: "none" }
        : deriveOctopusVisuals(node), [node, isOctoboss]);
    ...
    return (<g className={`canvas-node canvas-node--tentacle${showFocus ? " canvas-node--selected" : ""}`} data-node-id={node.id} transform={`translate(${node.x}, ${node.y})`} onPointerDown={(e) => {
            if (e.button !== 0)
                return;
            e.stopPropagation();
      ...
      {connectedNodes.map((target) => {
            ...
            return (<g key={target.id}>
            <path className="canvas-edge" d={path} fill="none" stroke={active ? (selectedNodeColor ?? color) : "#C0C0C0"} strokeWidth={active ? 2 : 1.5} strokeOpacity={1}/>
      ...
      <foreignObject x={-glyphW / 2} y={-glyphH / 2} width={glyphW} height={glyphH} style={{ overflow: "visible", pointerEvents: "none" }}>
        <div style={{
            display: "flex",
            alignItems: "center",
          ...
          <OctopusGlyph {...(isOctoboss ? {} : { color })} animation={visuals.animation} expression={visuals.expression} accessory={visuals.accessory} {...(visuals.hairColor ? { hairColor: visuals.hairColor } : {})} scale={glyphScale}/>
        </div>
      </foreignObject>

      {/* Label — always visible, up to two lines */}
      <text y={glyphH / 2 - 12} textAnchor="middle" className="canvas-node-label canvas-node-label--tentacle canvas-node-label--always" fill={isOctoboss ? "var(--accent-primary, #d4a017)" : "#faa32c"}>
        <tspan x="0" dy="0">
          {lines[0]}
        </tspan>
...
//# sourceMappingURL=OctopusNode.js.map