...
export const CanvasTentaclePanel = ({ node, isFocused, onClose, onFocus, panelRef, tentacle, sessions, onCreateAgent, onSolveTodoItem, onSpawnSwarm, onNavigateToConversation, onRefreshTentacleData, }) => {
    ...
    const progressPct = tentacle && tentacle.todoTotal > 0
        ? Math.round((tentacle.todoDone / tentacle.todoTotal) * 100)
        : 0;
    return (<div ref={panelRef} className={`detail-panel${isFocused ? " detail-panel--focused" : ""}`} tabIndex={-1} onPointerDown={() => onFocus?.()}>
      {/* Header */}
      <div className="detail-panel-header" style={{
        ...
        {tentacle && (<span className="detail-type-badge">
      ...
      <div className="detail-content">
        ...
        <div className="detail-identity">
          {visuals && (<div className="detail-glyph">
              <OctopusGlyph color={visuals.color} animation={visuals.animation} expression={visuals.expression} accessory={visuals.accessory} {...(visuals.hairColor ? { hairColor: visuals.hairColor } : {})} scale={6}/>
            </div>)}
          <div className="detail-identity-info">
            ...
            {tentacle?.description && (<div className="detail-row">
        ...
        <div className="detail-section">
          ...
          <div className="detail-actions">
            <button type="button" className="detail-action-btn" onClick={() => onCreateAgent?.(node.tentacleId)}>
              &gt;_ Create Agent
            </button>
            <button type="button" className="detail-action-btn" onClick={() => onSpawnSwarm?.(node.tentacleId, "worktree")}>
              &#x2263; Spawn Swarm (Worktrees)
            </button>
            <button type="button" className="detail-action-btn" onClick={() => onSpawnSwarm?.(node.tentacleId, "shared")}>
              &#x2263; Spawn Swarm (Normal)
            </button>
          </div>
        </div>
        ...
        {/* Progress section */}
        {tentacle && (<div className="detail-section">
            <div className="detail-section-title">Progress</div>
            {tentacle.todoTotal > 0 && (<div className="detail-progress">
                <div className="detail-progress-bar">
                  <div className="detail-progress-fill" style={{ width: `${progressPct}%`, backgroundColor: node.color }}/>
                </div>
                <span className="detail-progress-label">
                  {tentacle.todoDone}/{tentacle.todoTotal}
                </span>
              </div>)}
            {tentacle.todoItems.length > 0 && (<ul className="detail-todos">
                {tentacle.todoItems.map((item, i) => (<li key={`${i}-${item.text}`} className={`detail-todo${item.done ? " detail-todo--done" : ""}`}>
                    <div className="detail-todo-controls">
                      <button type="button" className="detail-todo-delete" title="Delete item" onClick={() => void handleTodoDelete(i)}>
                        <X size={12}/>
                      </button>
                      <button type="button" className="detail-todo-solve" aria-label={`Spawn agent for todo item: ${item.text}`} title="Spawn agent for this item" disabled={item.done || solvingTodoIndex === i} onClick={() => void handleTodoSolve(i)}>
                        {solvingTodoIndex === i ? "…" : <Terminal size={15} strokeWidth={2.4}/>}
                      </button>
                      <input type="checkbox" checked={item.done} onChange={() => handleTodoToggle(i, !item.done)}/>
                    </div>
                    {editingIndex === i ? (<input className="detail-todo-edit-input" type="text" value={editText} onChange={(e) => setEditText(e.target.value)} onKeyDown={(e) => {
                            if (e.key === "Enter")
                                void handleTodoEdit(i, editText);
                            if (e.key === "Escape")
                                setEditingIndex(null);
                        }} onBlur={() => void handleTodoEdit(i, editText)}/>) : (<span className="detail-todo-text" onDoubleClick={() => {
                            setEditingIndex(i);
            ...
            {addingTodo ? (<div className="detail-todo-add-row">
                <input className="detail-todo-edit-input" type="text" placeholder="New todo item…" value={addText} onChange={(e) => setAddText(e.target.value)} onKeyDown={(e) => {
                    if (e.key === "Enter")
              ...
              </div>) : (<button type="button" className="detail-todo-add-btn" onClick={() => setAddingTodo(true)}>
                + Add item
              </button>)}
          </div>)}

        {/* Vault files */}
        {tentacle && tentacle.vaultFiles.length > 0 && (<div className="detail-section">
            <div className="detail-section-title">Vault Files</div>
            <div className="detail-labels-list">
              {tentacle.vaultFiles.map((file) => (<span key={file} className="detail-label-tag">
                  {file}
                </span>))}
            </div>
          </div>)}

        {tentacle && tentacle.suggestedSkills.length > 0 && (<div className="detail-section">
            <div className="detail-section-title">Suggested Skills</div>
            <div className="detail-labels-list">
              {tentacle.suggestedSkills.map((skill) => (<span key={skill} className="detail-label-tag">
                  {skill}
                </span>))}
            </div>
          </div>)}
        ...
        <div className="detail-section">
          ...
          {sessions.length === 0 ? (<div className="detail-empty">No sessions yet</div>) : (<div className="detail-sessions">
              {sessions.map((s) => (<button key={s.sessionId} type="button" className="detail-session-item" onClick={() => onNavigateToConversation?.(s.sessionId)}>
...
//# sourceMappingURL=CanvasTentaclePanel.js.map