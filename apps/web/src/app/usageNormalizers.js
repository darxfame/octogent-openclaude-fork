...
export const normalizeCodexUsageSnapshot = (value) => {
...
export const normalizeClaudeUsageSnapshot = (value) => {
    const record = asRecord(value);
    if (!record) {
        return null;
    }
    ...
    const source = record.source === "cli-pty" ? "cli-pty" : record.source === "oauth-api" ? "oauth-api" : "none";
    return {
        status,
        source,
        fetchedAt: asString(record.fetchedAt) ?? new Date().toISOString(),
...
//# sourceMappingURL=usageNormalizers.js.map