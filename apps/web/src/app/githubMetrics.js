...
export const formatGitHubCommitHoverLabel = (point) => {
...
export const buildGitHubStatusPill = (summary) => {
    if (!summary) {
...
export const buildGitHubCommitSeries = (summary) => {
...
export const buildGitHubCommitSparkPoints = (series, width, height) => {
...
export const buildGitHubSparkPolylinePoints = (series) => series.map((point) => `${point.x.toFixed(1)},${point.y.toFixed(1)}`).join(" ");
export const buildGitHubCommitCount = (series) => series.reduce((total, point) => total + point.count, 0);
