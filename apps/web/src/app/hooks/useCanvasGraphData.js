...
const getAccentPrimary = () => (typeof document !== "undefined"
...
const tentacleColor = (tentacleId, deckColor) => deckColor && deckColor.length > 0
    ? deckColor
    : OCTOPUS_COLORS[hashString(tentacleId) % OCTOPUS_COLORS.length];
const buildTentacleNodeId = (tentacleId) => `t:${tentacleId}`;
const buildActiveSessionNodeId = (agentId) => `a:${agentId}`;
const buildInactiveSessionNodeId = (sessionId) => `i:${sessionId}`;
const normalizeDeckTentacleSummary = (value) => {
    ...
    const todoItems = Array.isArray(record.todoItems)
        ...
        : [];
    const scopeRecord = record.scope !== null && typeof record.scope === "object"
        ? record.scope
        : null;
    const octopusRecord = record.octopus !== null && typeof record.octopus === "object"
        ? record.octopus
        : null;
    ...
    return {
        ...
        scope: {
            paths: Array.isArray(scopeRecord?.paths)
                ? scopeRecord.paths.filter((path) => typeof path === "string")
                : [],
            tags: Array.isArray(scopeRecord?.tags)
                ? scopeRecord.tags.filter((tag) => typeof tag === "string")
                : [],
        },
        vaultFiles: Array.isArray(record.vaultFiles)
            ? record.vaultFiles.filter((file) => typeof file === "string")
            : [],
        todoTotal: typeof record.todoTotal === "number" && Number.isFinite(record.todoTotal)
            ? record.todoTotal
            : todoItems.length,
        todoDone: typeof record.todoDone === "number" && Number.isFinite(record.todoDone)
            ? record.todoDone
            : todoItems.filter((item) => item.done).length,
        todoItems,
        suggestedSkills: Array.isArray(record.suggestedSkills)
            ? record.suggestedSkills.filter((skill) => typeof skill === "string")
            : [],
    };
};
export const useCanvasGraphData = ({ columns, enabled, agentRuntimeStates, }) => {
    const [deckTentacles, setDeckTentacles] = useState([]);
    ...
    const sessionsByTentacleId = new Map();
    ...
    for (const terminal of columns) {
        if (terminal.tentacleId !== OCTOBOSS_ID)
            continue;
    ...
    for (const session of inactiveSessions) {
        if (!session.tentacleId || !seenTentacleIds.has(session.tentacleId))
            continue;
        if (activeTerminalIds.has(session.sessionId))
            continue;
...
//# sourceMappingURL=useCanvasGraphData.js.map