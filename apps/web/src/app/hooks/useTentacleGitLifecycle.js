...
import { buildTentacleGitCommitUrl, buildTentacleGitPullRequestMergeUrl, buildTentacleGitPullRequestUrl, buildTentacleGitPushUrl, buildTentacleGitStatusUrl, buildTentacleGitSyncUrl, } from "../../runtime/runtimeEndpoints";
...
const parseTentacleGitStatus = (payload) => {
...
const parseTentaclePullRequest = (payload) => {
    if (payload === null || payload === undefined || typeof payload !== "object") {
        return null;
    }
    const record = payload;
...
export const useTentacleGitLifecycle = ({ columns, }) => {
    ...
    const [openGitTentacleId, setOpenGitTentacleId] = useState(null);
    ...
    const fetchTentacleGitStatus = useCallback(async (tentacleId) => {
    ...
    const fetchTentaclePullRequest = useCallback(async (tentacleId) => {
    ...
    const worktreeTentacleIds = useMemo(() => columns
        .filter((column) => column.workspaceMode === "worktree")
        .map((column) => column.tentacleId), [columns]);
    useEffect(() => {
        const activeTentacleIds = new Set(columns.map((column) => column.tentacleId));
        setGitStatusByTentacleId((current) => Object.fromEntries(Object.entries(current).filter(([tentacleId]) => activeTentacleIds.has(tentacleId))));
        setGitStatusLoadingByTentacleId((current) => Object.fromEntries(Object.entries(current).filter(([tentacleId]) => activeTentacleIds.has(tentacleId))));
        setGitStatusAttemptedTentacleIds((current) => Object.fromEntries(Object.entries(current).filter(([tentacleId]) => activeTentacleIds.has(tentacleId))));
        setPullRequestByTentacleId((current) => Object.fromEntries(Object.entries(current).filter(([tentacleId]) => activeTentacleIds.has(tentacleId))));
        setPullRequestLoadingByTentacleId((current) => Object.fromEntries(Object.entries(current).filter(([tentacleId]) => activeTentacleIds.has(tentacleId))));
        setPullRequestAttemptedTentacleIds((current) => Object.fromEntries(Object.entries(current).filter(([tentacleId]) => activeTentacleIds.has(tentacleId))));
    ...
    useEffect(() => {
        for (const tentacleId of worktreeTentacleIds) {
            ...
            void fetchTentacleGitStatus(tentacleId).catch((error) => {
    ...
    useEffect(() => {
        for (const tentacleId of worktreeTentacleIds) {
            ...
            void fetchTentaclePullRequest(tentacleId).catch((error) => {
    ...
    const openTentacleGitActions = useCallback((tentacleId) => {
    ...
    const runGitMutation = useCallback(async (action, request = {}) => {
        ...
        try {
            ...
            setGitDialogError(error instanceof Error ? error.message : `Unable to ${action} tentacle worktree.`);
            return null;
        }
        finally {
    ...
    const openGitTentacleStatus = openGitTentacleId !== null ? (gitStatusByTentacleId[openGitTentacleId] ?? null) : null;
    const openGitTentaclePullRequest = openGitTentacleId !== null ? (pullRequestByTentacleId[openGitTentacleId] ?? null) : null;
    const isGitDialogLoading = openGitTentacleId !== null
...
//# sourceMappingURL=useTentacleGitLifecycle.js.map