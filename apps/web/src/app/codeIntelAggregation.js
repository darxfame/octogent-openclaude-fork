...
export const buildTreemapTree = (events, workspaceCwd) => {
...
export const layoutTreemap = (root, width, height) => {
    const rects = [];
    if (root.value === 0 || root.children.length === 0)
        return rects;
...
const worstRatio = (row, totalArea, shortSide) => {
...
export const buildCouplingData = (events, workspaceCwd) => {
    ...
    for (const e of events) {
        const relative = e.file.startsWith(workspaceCwd)
            ? e.file.slice(workspaceCwd.length + 1)
            : e.file;
        if (!sessionFiles.has(e.sessionId))
            sessionFiles.set(e.sessionId, new Set());
        sessionFiles.get(e.sessionId)?.add(relative);
        fileEdits.set(relative, (fileEdits.get(relative) ?? 0) + 1);
        if (!fileSessions.has(relative))
            fileSessions.set(relative, new Set());
...
export const heatColor = (value, maxValue) => {
    if (maxValue === 0)
        return "rgb(10,10,46)";
...
//# sourceMappingURL=codeIntelAggregation.js.map